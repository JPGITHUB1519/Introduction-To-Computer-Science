...
angular.module("core").config(["$locationProvider","$compileProvider",function(a,c){a.html5Mode(!1);a.hashPrefix("!");c.aHrefSanitizationWhitelist(/^\s*(https?|mailto|blob):/)}]).run(["dateTime","secondsSinceEpoch","$rootScope","$location","currentUser","$window",function(a,c,b,d,e,f){a.initialize(1E3*c);var g=readCookie("ajs_user_id");b.$on("$locationChangeSuccess",function(a,d,c){function m(a){return a.replace("-"," ").replace("_"," ").replace(/\w\S*/g,function(a){return a.charAt(0).toUpperCase()+
a.substr(1).toLowerCase()})}a=!1;-1!==d.indexOf("#!")&&(-1===c.indexOf("#!")&&d.replace("#!","").replace(/\/$/,"")===c.replace(/\/$/,""))&&(a=!0);var l=d.replace("#!",""),l=l.replace(/\/$/,""),l=URI.parse(l),k=l.path.toLowerCase();""!==k&&!/^\/(me|courses|nanodegree|course|account|checkout)/.test(k)&&(a=!0);var p=e.getUser(),q=p&&p.key||"Anonymous";if(p&&p._registered&&p.key){var n;if(_.isNull(g)||"null"===g)n={firstName:p.first_name,lastName:p.last_name},p.email&&p.email.address&&(n.email=p.email.address),
analytics.identify(p.key,n),g='"'+p.key+'"'}var p=null,r=/^\/course\/.*--.*$/i,u=/^\/course\/viewer/i;n=null;var w=/^\/!c-nd\d{3}.*/i,t=/^\/course\/.*--nd\d{3}$/i;""===k?n=p="Home":"/account"===k.substring(0,8)?"/account/auth/signin"===k?n=p="Sign In":"/account/auth/signup"===k?n=p="Sign Up":(p="My Account Settings",n="My Account Settings - "+q+" - "+(m(k.substring(9,k.length))||"Contact Info")):"/me"===k.substring(0,3)?(p="My Home",n=p+" - "+q):"/nanodegree"===k?n=p="Nanodegrees":"/courses"===k.substring(0,
8)?(p="Catalog",n="Catalog - "+m(k.substring(9,k.length))):"/checkout"===k.substring(0,9)?(p="Checkout",n="Checkout - "+q):t.test(k)?(p="Nanodegree Info",n="Nanodegree Info - "+k.split("--")[1]):u.test(k)?(p=/c-([^/]+)/.exec(k),n="unknown",2<=p.length&&(n=p[1]),w.test(k)?(p="My Nanodegree Classroom",n="My Nanodegree Classroom - "+q+" - "+n):(p="My Course Classroom",n="My Course Classroom - "+q+" - "+n)):r.test(k)&&(p="Course Info",n="Course Info - "+k.split("--")[1]);var l={category:p,path:k,search:l.query,
url:d},s={},q=function(a){var b={};b.conceptKey=s.exercise?a.morsel:a.exercise;b.atomKey=a.morsel;b.lessonKey=a.lesson;"nd"==s.course.toLowerCase().substring(0,2)?(b.degreeKey=a.course,b.projectKey=null,b.moduleKey=null):b.courseKey=a.course;return b};if("My Nanodegree Classroom"===p||"My Course Classroom"===p)_.each({course:/c-([^/]+)/,lesson:/l-(\d+)/,morsel:/m-(\d+)/,exercise:/e-(\d+)/},function(a,b){var d=a.exec(k);d&&d.length&&(s[b]=d[1])}),null==s.morsel?a=!0:_.extend(l,q(s));l.referrer=d===
c?document.referrer:c;b.segmentPageProperties={page:l};f.segmentPageProperties={page:l};a||analytics.page(p,n,l)})}]);angular.module("page.account",["ngRoute","core","account"]);
...
angular.module("account").controller("accountPage",["$scope","$location",function(a,c){a.isActive=function(a){return c.path()?c.path()===a:"/contact-info"===a}}]);angular.module("account").controller("billingAndSubscriptionsTab",["$scope","currentUser","modal","stripeCreditCard","customers",function(a,c,b,d,e){a.loading=!0;a.editingCreditCard=!1;a.enabledCreditCardButton=!0;a.creditCardErrorMessage="";a.removePaymentCard=function(d){a.loading=!0;return null===a.customer.subscriptions||0===a.customer.subscriptions.length?b.activate("removeCard").then(function(){return e.removeCreditCard("me",d).then(function(){a.customerCard=null;a.serverSuccess=!0},function(){a.serverError=
!0})})["finally"](a.loading=!1):b.activate("cannotRemoveCard").then(function(){a.editingCreditCard=!0})["finally"](a.loading=!1)};c=function(){a.loading=!0;return e.get("me").then(function(b){a.loading=!1;b&&(_.each(b.subscriptions,function(a){var b=a.plan.amount;if(a.discount){var d=a.discount.coupon;d.percent_off&&(b-=b*d.percent_off/100);d.amount_off&&(b-=d.amount_off)}a.nextChargeAmount=b}),a.customer=b,a._addSimplifiedSubscriptionMetadata(a.customer.subscriptions),a.customerCard=d.getDefaultCard(b),
a.creditCard={})})};a.$on("refreshPageData",c);a._addSimplifiedSubscriptionMetadata=function(a){return _.each(a,function(a){a.setForCancellation=a.cancel_at_period_end;a.isLive="trialing"!==a.status||a.plan.metadata.trial_period_days&&0<a.plan.metadata.trial_period_days})};a.updateCreditCard=function(){var b=d.checkValidity(a.creditCard);a.creditCardErrors=b.errorText;b.valid&&(a.enabledCreditCardButton=!1,d.createPaymentToken("Update Credit Card",a.creditCard).then(function(a){return a.id}).then(function(a){return e.put("me",
{stripe_token:a})}).then(function(b){a.creditCardErrorMessage="";a.customer=b;a.customerCard=d.getDefaultCard(b);a.editingCreditCard=!1;a.enabledCreditCardButton=!0},function(b){a.creditCardErrorMessage=d.prettifyErrorResponse(b);a.enabledCreditCardButton=!0}))};c()}]);angular.module("account").factory("cancellationFlow",["modal","$q","api","$location","customers","CancellationFrame","products","recordActivity","dateTime","currentUser","$rootScope","$window",function(a,c,b,d,e,f,g,h,i,j,m,l){var k=j.fetchFullUser();return{cancelSubscription:function(d){var c=d.id,e=d.status,h=1E3*(d.trial_end||d.start),j=1E3*d.current_period_end,m=(Math.random().toString(36)+"000000000000000").slice(2,16),t=d.plan.id;"trialing"!==e&&i.calculateDaysBetweenDates(new Date(h),i.getCurrent());
f.setFlowToken(m);f.sendCancellationActivity("start");return k.then(function(){return g.get(t).then(function(a){try{return f.setCourseKey(a._course_groups[0].course_ref.key),b.get("/users/node_progress/"+f.getCourseKey()).then(function(a){f.setCompletionStats(a.data.stats);return a.data.stats})}catch(d){return f.setCourseKey("ndXXX")}})}).then(function(){return a.activate("postCancelSurvey",{subscriptionId:c,subscriptionPlanId:t,subscriptionPeriodEndDate:new Date(j)}).then(function(){l.location.reload()}).catch(function(){f.sendCancellationActivity("abort").then(function(){f.setCurrentFrame("cancelSurvey")})})},
function(){l.location.reload()})}}}]);angular.module("account").factory("CancellationFrame",["recordActivity",function(a){var c="cancelSurvey",b="",d="",e={time:0,projects:0,quizzes:0},f=function(){switch(c){case "cancelSurvey":return 1;case "accomplishments":return 2;case "hereToHelp":return 3;case "startAConvo":return 4;case "rogerThat":return 5;case "reminder":return 6;case "cancelled":return 7}},g=function(){var a=d.slice(0,2);return"ca"===a?"Career":"nd"===a?"Degree":"Course"};return{getCurrentStep:f,getCurrentFrame:function(){return c},
setCurrentFrame:function(a){return c=a},getFlowToken:function(){return b},setFlowToken:function(a){return b=a},getCourseKey:function(){return d},setCourseKey:function(a){return d=a},getCourseType:g,sendCancellationActivity:function(c,e,j,m){var l=b,e=e||f(),k=d,p=g(),q,n;j&&(q=j);m&&(n=m);return a.cancellationFlowStep(l,e,k,p,c,q,n)},getCompletionStats:function(){return e},setCompletionStats:function(a){e={time:Math.round(a.time/60),projects:a.projects,quizzes:a.quizzes}},sufficientTimeInClass:function(){return 30<=
...
a.errorText;a.valid?g.put(c.user).then(function(){c.serverSuccess="Employer Information Saved";c.saveDisabled=!1;b.location.reload()},function(a){c.serverError=a.error;c.saveDisabled=!1}):c.saveDisabled=!1}}}]);angular.module("account").controller("linkedAccountsTab",["$scope","$timeout","currentUser","users","google","facebook","normalizedSocialService","session","siteExperience",function(a,c,b,d,e,f,g,h,i){a.loading=!0;a.managed=i.managedAccount;a.accounts={google:null,facebook:null};var j=function(){a.serverSuccess="";a.serverError=""},m=function(b,d,c){d=g.normalizeAccount(b,d);d.id===a.user["_"+b+"_id"]||c?d.loggedIn=!0:d={loggedIn:!1};return a.accounts[b]=d},l=function(b){a.loading=!1;a.serverError=
b.error};a.getLinkedAccountTypes=function(){return _.keys(a.accounts)};a.disconnect=function(b){a.loading=!0;var c={key:a.user.key};c[b+"_id"]=null;d.put(c).then(function(d){j();a.loading=!1;a.user=d;"github"!==b&&(a.accounts[b]=null)},l)};a.disconnectSocial=function(b){a.loading=!0;var c={key:a.user.key},e=a.user.social_accounts.length&&"github"==b?"social_accounts":"external_accounts",f=null;_.each(a.user[e],function(a,d){if(a.account_type===b||a.affiliate_program_key===b)f=d});null==f?a.loading=
!1:(a.user[e].splice(f,1),c[e]=a.user[e],d.put(c).then(function(d){j();a.loading=!1;a.user=d;a[b]=null},l))};a.connect=function(b){a.loading=!0;var e=g.normalizeAccountService(b),f=function(g,h){var i=function(a){401===a.code&&f(g-1,3*h)};0===g&&(i=l);c(function(){e.getAccount().then(function(c){var e={key:a.user.key};e[b+"_id"]=c.id;d.put(e).then(function(d){c=m(b,c,!0);j();a.loading=!1;a.user=d},l)},i)},h)},i=function(){f(3,500)};e.login().then(function(a){a=g.createSessionData(b,a);h.put(a).then(i,
i)},l)};j();b.fetchFullUser().then(function(){a.loading=!1;a.user=b.getUser();_.each(a.user.social_accounts,function(b){a[b.affiliate_program_key]=b});_.each(a.user.external_accounts,function(b){a[b.affiliate_program_key]=b});_.each(a.getLinkedAccountTypes(),function(a){g.normalizeAccountService(a).getAccount().then(function(b){m(a,b)})})},l)}]);angular.module("account").controller("myCertificatesTab",["$q","$scope","$sce","currentUser","users","nodes",function(a,c,b,d,e,f){c.loading=!0;c.verifiedCertificates=[];c.nodeMap={};c.serverError="";var g=d.getUser();c.getBadges=function(){return _.filter(g._badges,function(a){return"Course Mastery"===a.name&&0<a.highest_level})};a.all([e.getCertificates("me").then(function(a){c.verifiedCertificates=a;a=_.map(a,function(a){return a.node_ref.key});return f.batchGet(a,{projection:"catalog"}).then(function(a){_.extend(c.nodeMap,
...
6):c}}]).filter("stringDate",["$filter",function(){var a=function(a){if(!a)return a;var b=a.lastIndexOf(".");if(-1===b)return a;var d=b+7;if(d>a.length)return a;for(var e=b+1;e<d;++e)if("0">a[e]||"9"<a[e])return a;return a.substring(0,b)+a.substring(d,a.length)};return function(c){return Date.parse(a(c))}}]);angular.module("filters").filter("formatCurrency",[function(){return function(a,c){if("object"==typeof Intl&&Intl&&"function"==typeof Intl.NumberFormat){if("USD"===c)return(a/100).toLocaleString("en-US",{style:"currency",currency:c})+" "+c;if("INR"===c)return(a/100).toLocaleString("en-IN",{style:"currency",currency:c})+" "+c}else{var b=0>a?"-":"",a=Math.abs(a);if("USD"===c){var d=a%100;return b+"$"+Math.floor(a/100)+(d?"."+d:"")+" "+c}if("INR"===c)return b+"\u20b9"+Math.floor(a/100)+" "+c}return a+
c}}]);angular.module("filters").filter("formatCurrency2",[function(){return function(a,c){var b;if("object"==typeof Intl&&Intl&&"function"==typeof Intl.NumberFormat)"USD"===c?b=(a/100).toLocaleString("en-US",{style:"currency",currency:c}):"INR"===c&&(b=(a/100).toLocaleString("en-IN",{style:"currency",currency:c}));else{var d=0>a?"-":"",a=Math.abs(a);"USD"===c?(b=a%100,b=d+"$"+Math.floor(a/100)+(b?"."+b:"")):"INR"===c&&(b=d+"\u20b9"+Math.floor(a/100))}return b?/\.00$/.test(b)?b.slice(0,-3):b:a+c}}]);angular.module("filters").filter("imageCropSize",[function(){return function(a,c){if(a)return a.match(/\/\/[^.\/]+\.(ggpht|googleusercontent)\.com\//)||a.match(/\/_ah\/img\//)?a=a.replace(/\=s\d+(-c)?/,"=s"+c+"-c"):a.match(/\/\/robohash\.org\//)&&(a=a+"?size="+c+"x"+c),a}}]).filter("imageSize",[function(){return function(a,c,b){if(a){b=0===b?void 0:b;c=0===c?void 0:c;if("number"!==typeof c&&"number"!==typeof b)return a;var d=Math.max(b,c)||b||c;if(a.match(/\/\/[^.\/]+\.(ggpht|googleusercontent)\.com\//)||
...
isFreeEnrolled:function(a,d){return!c.isCurrentEnrolled(a,d)&&"enrolled"===a.state},hasEnrolled:function(b,d){return a(b,d,"past_student_group_refs")||c.isGraduated(b,d)},hasPermissionTo:function(a,d){return a&&!!_.find(a.guard.allowed_behaviors,function(a){return a===d})}};return c}]);angular.module("foundation").directive("notificationsDropdown",["notificationsDropdownHelper","layer","$q","currentUser","$timeout","$window","nodes","cacheVersion",function(a,c,b,d,e,f,g,h){return{scope:{},restrict:"A",templateUrl:"/templates/notifications_dropdown/notifications_dropdown.html?"+h,link:function(b){var h;c.authenticate().then(function(){h=c.startWebSocketConnection();return d.fetchStudyGroups()}).then(function(d){b.showNotifications=d&&0<d.length;if(b.showNotifications){b.loading=
!0;b.nodeMap=null;b.hasSeenAllNotifications=!1;b.markNotificationsAsRead=function(){b.hasSeenAllNotifications=!0};b.hasUnreadNotification=function(){return _.some(b.nodeMap,function(a){return a.conversation&&0<a.conversation.unread_message_count})};var c=_.flatten(_.map(d,function(a){return _.map(a._node_refs,function(a){return a.key})}));b.openTeamsPage=function(a){a=b.goToTeamsPage(a);f.open(a,"_blank")};b.goToTeamsPage=function(a){return a?"/teams/#/chat/"+a:"/teams/#/chat/"+c[0]};var k=_.indexBy(d,
"conversation_id");g.batchGet(c).then(function(c){b.nodeMap=c;a.updateNodeMapWithExistingConversations(b.nodeMap,d).then(function(){e(function(){})});b.loading=!1;var f=0;h.onmessage=function(d){d=JSON.parse(d.data);a.isUnreadMessageCountPacket(d)&&(a.updateUnreadMessageCountInNodeMap(b.nodeMap,k,d),b.hasSeenAllNotifications&&(d=a.calculateTotalUnreadMessageCountInNodeMap(b.nodeMap),d>f&&(b.hasSeenAllNotifications=!1),f=d),e(function(){}))}})}})}}}]).service("notificationsDropdownHelper",["layer",
"$q",function(a,c){return{isUnreadMessageCountPacket:function(a){return a.object&&"Conversation"===a.object.type&&"change"===a.type?_.some(a.data,function(a){return"unread_message_count"===a.property}):!1},updateNodeMapWithExistingConversations:function(b,d){return c.all(_.map(d,function(d){return a.fetchConversation(d.conversation_id).then(function(a){b[d._node_refs[0].key].conversation=a}).catch(function(){})}))},updateUnreadMessageCountInNodeMap:function(a,d,c){if(d=d[c.object.id]){var f=a[d._node_refs[0].key];
f&&(f.conversation=f.conversation||{},_.forEach(c.data,function(a){"unread_message_count"===a.property&&(f.conversation.unread_message_count=a.value)}))}return a},calculateTotalUnreadMessageCountInNodeMap:function(a){return _.reduce(a,function(a,b){return b.conversation&&b.conversation.unread_message_count?a+b.conversation.unread_message_count:a},0)}}}]).service("layer",["$http","$timeout","$q","applicationEnvironment","users",function(a,c,b,d,e){var f={};f.layerAppId=d.isInDevelopment()?"e5fe3bb6-1ef1-11e5-9078-ad56000000fb":
"e5fe4034-1ef1-11e5-8bd1-ad56000000fb";f.fetchConversation=function(a){return f.layerGet(f.getUrlFromLayerId(a)).then(function(a){return a.data})};f.getUrlFromLayerId=function(a){return a.split("layer://")[1]};f.layerGet=function(b,d,c){return a.get("https://udacity-api.layer.com"+b,_.extend({data:d},{headers:f.addLayerHeaders(_.extend(a.defaults.headers.common,c))}))};f.layerPost=function(b,d){return a.post("https://udacity-api.layer.com"+b,d,{headers:f.addLayerHeaders(a.defaults.headers.common)})};
...
function(){return new WebSocket("wss://udacity-api.layer.com/websocket?session_token="+f.sessionToken,"com.layer.notifications-1.0")};f.requestAuthenticationNonceWithCompletion=function(a){return e.fetchJWTForLayer(a).then(function(a){return f.layerPost("/sessions",{identity_token:a,app_id:f.layerAppId})}).then(function(a){return a.data.session_token})};return f}]);angular.module("foundation").controller("siteFrame",["$scope","$window","localStorage","Amplitude","currentUser",function(a,c,b,d,e){a.responsivenessIsEnabled=!1;a.$on("setResponsiveness",function(b,d){!a.responsivenessIsEnabled&&d&&0===$('meta[name="viewport"]').length?$("head").append('<meta name="viewport" content="width=device-width, initial-scale=1.0" />'):$('meta[name="viewport"]').remove();a.responsivenessIsEnabled=d});a.cacheBustingIsEnabled="yes"===b.getItem("bustAllTheCaches")?"yes":"no";
a.toggleCacheBusting=function(){a.cacheBustingIsEnabled="yes"===a.cacheBustingIsEnabled?"no":"yes";b.setItem("bustAllTheCaches",a.cacheBustingIsEnabled);c.location.reload()};angular.element(document).ready(function(){e.discoverAndSetCurrency()})}]);angular.module("foundation").directive("tocPageTocGenerator",[function(){return{restrict:"A",link:function(a,c,b){a.tocEntries=[];var d=[],e="";try{e=JSON.parse(b.tocSelector.replace(/\'/g,'"'))}catch(f){e=[b.tocSelector]}if(_.isArray(e)){_.each(e,function(a,b){c.find(a).each(function(a){$(this).attr("id","section-"+b+"-"+a)})});c.find(e.join(", ")).each(function(){d.push({id:$(this).attr("id"),name:$(this).text(),level:parseInt(/section-(\d*)-\d*/.exec($(this).attr("id"))[1],10)})});var g=function(a){for(var b=
...
"localStorage",function(f,g,h,i,j,m){var l=["sessionExpired"];f.defaults.headers.common["X-Udacity-Client"]="udacity.com; version="+i;var k=function(d){var i={get:function(b,e,g){g=g||{};"yes"===m.getItem("bustAllTheCaches")&&(e=e||{},e.cacheBusterValue=Math.random());g.params=a(e);g.cache=!0;var h=i.buildGetUrl(b,e),j=c();return 1600<h.length?(g.lengthy||console.warn("Url potentially too large for GET request; switching to POST. Set configuration option lengthy=true to suppress this warning."),delete g.params,
...
(_.each(_.filter(_.values(d),b),function(b){a.stash(b.key,f,i,b)}),d):{}});var j={},m=[];_.each(d,function(b){var d=a.getFilledEntity(b,f,i);d?j[b]=d:m.push(b)});return c.when(j).then(function(b){return 0<m.length?g(m,h).then(function(d){d&&_.each(_.filter(_.values(d),function(a){return a}),function(d){a.stash(d.key,f,i,d);b[d.key]=d});return b}):b})},standardMixin:function(a,b){var c=_.omit(b,"get","batchGet");c.__gateway=b;c.get=function(c,g){return g&&g.__skipCache?b.get(c,g):d.get(c,a,b.get,g,
...
a+"/is_reviewer").then(function(a){return a.data.is_reviewer})}}}]);angular.module("embedded").constant("tags",{collection:"k-",course:"c-",lesson:"l-",exercise:"e-",morsel:"m-",tidbit:"t-"}).constant("typeForTag",{"k-":"collection","c-":"course","l-":"lesson","e-":"exercise","m-":"morsel","t-":"tidbit"});angular.module("utility").directive("qualarooSurveyInfoHack",["qualarooSurveyor","dateTime","currentUser","cookieJar","qualaroo",function(a,c,b,d,e){return{link:function(){if(b.userIsLoggedIn()){var d=c.getCurrent(),g=new Date(d.setDate(d.getDate()-21));a.fetchEarliestPaidEnrollmentDate(b.getUser()).then(function(d){e.set({paid:d?"yes":"no"});e.set({subscribed_over_three_weeks:d&&d<g?"yes":"no"});e.set({gt:a.everEnrolledInGtCourses(b.getUser())?"yes":"no"})})}}}}]);angular.module("utility").factory("qualarooSurveyor",["dateTime","$q","nodeUtility","nodes",function(a,c,b,d){var e={fetchEarliestPaidEnrollmentDate:function(a){var g=_.without(_.filter(a._enrollments,function(a){return a.first_paid_enrollment_time},null,void 0));return g.length?d.batchGet(_.pluck(g,"node_key")).then(function(a){var d=_.filter(g,function(d){return _.any(a,function(a){return b.isCurrentEnrolled(d,a)})}),d=_.map(d,function(a){return e._convertIsoDateToYearMonthDayDate(a.first_paid_enrollment_time)});
return d.length?_.min(d):null}):c.when(!1)},_convertIsoDateToYearMonthDayDate:function(a){if(a)return a=a.substring(0,10).split("-"),a=_.map(a,function(a){return parseInt(a,10)}),new Date(a[0],a[1]-1,a[2])},everEnrolledInGtCourses:function(a){return _.any(a._enrollments,function(a){return _.any(a.group_refs,function(a){return a.key&&0===a.key.toLowerCase().indexOf("gt")})})}};return e}]);angular.module("overview").directive("careerEnrollmentActions",["cacheVersion","currentUser","groups","$location","dateTime","$window","cookieJar","googleTagManager","universalAnalytics",function(a,c,b,d,e,f,g,h,i){return{restrict:"A",scope:{enrollmentActions:"="},templateUrl:"/templates/overview/career_enrollment_actions.html?"+a,controller:"enrollmentActionsHelper",link:function(a){var b=function(){h.pushTrackingEvent("careerAccess");f.location.href="/me"};a.getCareerButtonInfo=function(){return a.isAccessible&&
...
a.trackViewedProduct(a.enrollmentActions)}}}]);angular.module("courseCatalog").config(["$locationProvider",function(a){a.html5Mode(!0).hashPrefix("!")}]).constant("SEARCH_DEBOUNCE_MILLIS",5E3).controller("courseCatalog",["$scope","$location","$timeout","$filter","$q","affiliates","courseCatalogHelper","publicCatalog","SEARCH_DEBOUNCE_MILLIS","universalAnalytics",function(a,c,b,d,e,f,g,h,i,j){var m=_.debounce(function(a,b,d){j.trackEvent({category:a,action:b,label:d})},i);a.fullTextSearch={query:""};a.filterCourses=function(b){var e=_.object(_.map(a.filters,
function(a,b){return[b,_.reduce(a.options,function(a,b){return b.selected?a.concat(b):a},[])]})),f=_.filter(b,function(b){return a.activeCategory&&_.any(a.activeCategory.all_children,function(a){return a&&a===b.key})&&_.every(a.filters,function(a,d){return!e[d].length||a.match(b,e[d])})});if(a.fullTextSearch.query){var g=$.trim(a.fullTextSearch.query).toLowerCase();m("Catalog","Search",g);f=d("filter")(f,a.fullTextSearch.query)}!f.length&&a.activeCategory?(f=_.filter(b,function(b){return _.any(a.activeCategory.courses,
function(a){return a&&a===b.key})}),a.fallback=!0):a.fallback=!1;b=_.sortBy(_.filter(a.filters,function(a){return a.exposeInAddressAs}),"id");_.each(b,function(a){var b=_.sortBy(_.pluck(_.where(a.options,{selected:!0}),"name"),_.identity);c.search(a.exposeInAddressAs,b)});return f};var l=null;a.getSelectedTechnologyNames=function(){l||(l=_.findWhere(a.filters,{id:"technologyKeywords"}));return _.map(_.pluck(_.where(l&&l.options,{selected:!0}),"name"),d("titleCase"))};a.getPageDescription=function(){var b=
a.getSelectedTechnologyNames(),d="",c=a.activeCategory&&a.activeCategory.name;1===b.length?d=_.first(b)+" ":1<b.length&&(d=_.initial(b).join(", ")+", and "+_.last(b)+" ");return"Browse "+c+" Course and Nanodegree Programs to help advance your career. You'll learn "+d+"from industry experts from Google, Facebook, Twitter and Cloudera."};h.list().then(function(b){var d=function(a){return!1!==a.public_listing};b.degrees=_.filter(b.degrees,d);b.courses=_.filter(b.courses,d);a.courses=_.map(_.union(b.degrees,
...
function(a){return a[1].last_interaction_time}).reverse()[0][0]);b=c}else b=null;a.lastInteractedLessonKey=b}),i.getCertificates("me").then(function(a){m=a;_.each(m,function(a){var b=a.node_ref.key;_.has(l,b)?l[b].last_modification_time<a.last_modification_time&&(l[b]=a):l[b]=a})})]).then(function(){a.loading=!1;a.premiumAccess=h.hasPermissionTo(a.course,"view-paid-supplemental-media")||h.isCurrentEnrolled(a.enrollment,a.course)||h.isGraduated(a.enrollment,a.course)})});a.getCompletedLessonKeys=function(a){return a?
...
b,controller:"passwordReset"})}]);angular.module("account").controller("accountAuth",["$scope","$location",function(a,c){a.initialSearch=c.search();c.replace().url(c.path())}]);angular.module("account").controller("passwordReset",["$scope","$sce","validator","users",function(a,c,b,d){var e=a.initialSearch.code,f=a.initialSearch.user_key;a.submit=function(){if(!a.buttonDisabled){a.buttonDisabled=!0;a.fieldErrors=null;a.errorMessage=null;a.successMessage=null;var g=b.check({password:{pretty:"your new password",value:a.password,validators:[{type:"required",chained:[{type:"strLen",args:[7,void 0]}]}]},passwordConfirm:{pretty:"confirmed password",value:a.passwordConfirm,validators:[{type:"required",
...
function(b){b=b.status;a.buttonDisabled=!1;a.errorMessage=400===b?c.trustAsHtml("We couldn't find an account with that email."):409===b?c.trustAsHtml("We have recently sent a reset email to that address. It may have been marked as spam."):c.trustAsHtml("Something went wrong and we were unable to send your reset email.")}):(a.fieldErrors=e.errorText,a.buttonDisabled=!1)}}}]);angular.module("account").controller("signInForm",["$scope","$q","$sce","session","users","currentUser","groups","validator","google","facebook","$location","safelyRedirect",function(a,c,b,d,e,f,g,h,i,j,m,l){var k=a.initialSearch.next;f.userIsLoggedIn()&&l(k);a.showTopBox=a.initialSearch.showMessage;var p=function(b){try{a.signedInUserAccountKey=b.data.account.key,a.signedInUserEmail=b.config.data.udacity.username}catch(d){console.log("Cannot retrieve accountKey and username"),a.signedInUserAccountKey=
null,a.signedInUserEmail=null}var b=a.initialSearch.enroll,e=a.initialSearch.enroll_state||"enrolled",h=a.initialSearch["join-group"],b=b?f.enroll(b,e,!0):c.when(!0),i=h?f.fetchFullUser().then(function(){return g.addMember(h,f.get("key"))}):c.when(!0);return b.then(function(){return i.then(function(){var b=f.getUser();analytics.identify(b.key,{email:a.signedInUserEmail},void 0,function(){analytics.track("Signed In",{accountKey:a.signedInUserAccountKey,category:"Sign In"},a.segmentPageProperties,function(){l(k)})})})})},
q=function(d){a.errorMessage=b.trustAsHtml("We were unable to validate your account.");return c.reject(d)},n=function(d){if(403===d.data.status)a.errorMessage=b.trustAsHtml('Your social account isn\'t connected yet, you can sign in with an existing account, or <a href="#!/signup">Sign Up</a> to get started.');else{if(502===d.data.status)return q(d);a.errorMessage=b.trustAsHtml("Something went wrong and we're currently looking into it.")}return c.reject(d)};a.signInEmail=function(){a.buttonDisabled=
!0;a.fieldErrors=null;a.email=$("#email").val();a.password=$("#password").val();var e=h.check({email:{pretty:"email address",value:a.email,validators:[{type:"required",chained:[{type:"email"}]}]},password:{pretty:"password",value:a.password,validators:[{type:"required"}]}});e.valid?d.put({udacity:{username:a.email,password:a.password}}).then(p,function(d){a.buttonDisabled=!1;a.errorMessage=b.trustAsHtml("The email or password you entered is invalid");return c.reject(d)}):(a.fieldErrors=e.errorText,
a.buttonDisabled=!1)};a.signInFacebook=function(){return j.getAuthResponse().then(function(a){return d.put({facebook:{signed_request:a.signedRequest}}).then(p,n)},q)};a.signInGoogle=function(){i.getToken(!1).then(function(a){return d.put({google:{token:a.code}}).then(p,n)},q)};a.hasLongCookieString=function(){return 3E3<document.cookie.length}}]);angular.module("account").controller("signUpForm",["$scope","$q","$sce","$timeout","session","users","currentUser","groups","validator","google","facebook","$location","safelyRedirect","googleTagManager","retry","cookieJar","universalAnalytics",function(a,c,b,d,e,f,g,h,i,j,m,l,k,p,q,n,r){a.signUpFormFillTracked=!1;a.$watchGroup(["email","password","firstName","lastName"],function(b){!a.signUpFormFillTracked&&_.any(b,function(a){return void 0!==a})&&analytics.track("Sign Up Started",{category:"Sign Up"},
a.segmentPageProperties,function(){a.signUpFormFillTracked=!0})});a._removeEmailCookie=function(){n.exists("emailAddressForSignUp")&&n.remove("emailAddressForSignUp",{path:"/"})};var u=function(b){a.registeredUser=b;var d=function(){var b=a.initialSearch.next;b?k(b):k("/")},b=a.initialSearch.enroll,e=a.initialSearch.enroll_state||"enrolled",f=a.initialSearch["join-group"],i=b?g.enroll(b,e,!0):c.when(!0),j=f?g.fetchFullUser().then(function(){return h.addMember(f,g.get("key"))}):c.when(!0),l,m;a._removeEmailCookie();
l=b?r.trackEvent({category:"SignUp",action:"Enroll",label:b}):r.trackEvent({category:"SignUp",action:"Direct"});m=p.pushTrackingEvent("RegistrationCompleted");var u=g.getUser();analytics.identify(a.registeredUser.key,{firstName:a.registeredUser.first_name,lastName:a.registeredUser.last_name,createdAt:new Date,email:a.registeredUser.email&&a.registeredUser.email.address},void 0,function(){analytics.track("Signed Up",{accountKey:u.key,category:"Sign Up"},a.segmentPageProperties,function(){return i["finally"](function(){return j["finally"](function(){return l["finally"](function(){return m["finally"](d)})})})})})},
w=function(d){a.errorMessage=b.trustAsHtml("We were unable to validate your account.");return c.reject(d)},t=function(d){a.errorMessage=b.trustAsHtml("We were unable to access your account.");return c.reject(d)},s=function(d){409===d.data.status?(a.errorMessage=b.trustAsHtml("This account already exists, please sign in to continue."),a._removeEmailCookie()):a.errorMessage=b.trustAsHtml("Something went wrong and we're currently looking into it.");a.buttonDisabled=!1;return c.reject(d)};a.signUpEmail=
...
first_name:a.firstName,last_name:a.lastName,password:a.password,tos_accepted:!0}).then(u,s):(a.fieldErrors=b.errorText,a.buttonDisabled=!1)};a.signUpFacebook=function(){return m.getAuthResponse().then(function(a){return m.getMeResponse().then(function(b){return f.registerUser({email_address:b.email,first_name:b.first_name,last_name:b.last_name,facebook:{signed_request:a.signedRequest},tos_accepted:!0}).then(u,s)},t)},w)};a.signUpGoogle=function(){j.getToken(!1).then(function(a){return e.put({google:{token:a.code}}).then(u,
function(){return q(j.getMeResponse,5,100).then(function(a){return j.getToken(!0).then(function(b){return f.registerUser({email_address:a.email,first_name:a.given_name,last_name:a.family_name,google:{token:b.code},tos_accepted:!0}).then(u,s)},w)},t)})},w)};n.exists("emailAddressForSignUp")&&(a.email=n.get("emailAddressForSignUp"))}]);angular.module("account").controller("viewerSettings",["$scope","currentUser","users","techDetect","$window",function(a,c,b,d){a.isBrowserIE="Explorer"===d.browserStats().browser;a.videoQualities=[{label:"Automatic",value:"default"},{label:"360p",value:"medium"},{label:"480p",value:"large"},{label:"720p",value:"hd720"},{label:"1080p",value:"hd1080"}];a.playbackRates=[{label:"0.25x",value:0.25},{label:"0.5x",value:0.5},{label:"Normal",value:1},{label:"1.5x",value:1.5},{label:"2x",value:2}];var e=c.getUser();
...
a.save=function(){a.serverSuccess=a.serverError="";a.saving=!0;e.site_preferences={use_html5_player:a.settings.useHtml5Player,auto_advance:a.settings.autoAdvance,playback_rate:a.settings.playbackRate.value,video_quality:a.settings.videoQuality.value};b.put(e).then(function(){a.saving=!1;a.serverSuccess="Classroom Settings Saved"},function(b){a.saving=!1;a.serverError=b.error})}}]);angular.module("payment").controller("checkoutPage",["$scope","paymentFlow","$routeParams","$location","api","currentUser","cacheVersion","dateTime","$timeout","universalAnalytics","$window",function(a,c,b,d,e,f,g,h,i,j,m){f.get("_registered")||(window.location.href="/account/auth#!/signup?next="+encodeURIComponent(window.location.href));a.$on("$routeChangeSuccess",function(){j.trackVirtualPageView();if(angular.isDefined(b.stepNumber)&&c.isInFlow())c.goToStep(b.stepNumber);else{var e=[];d.search().products?
(e=d.search().products.split(","),c.begin(e),a.flow=c.getCurrentFlowData(),a.flow.abandonedCheckout=!0):window.history.back()}});a.getCurrentDate=function(){return h.getCurrent()};a.isProductLaunched=function(a){return a&&a.launch_date&&new Date(a.launch_date)<h.getCurrent()};a.isProductNotLaunched=function(a){return a&&!a.was_launched};a.getGoToNanodegreeLink=function(b){return b&&b.shouldSeeOnboarding?"/teams/#/onboarding/"+a.flow.uriEncodedCourseKey:"/me"};a.getDescStringByCourseType=function(a){switch(a){case "Degree":return"Nanodegree";
case "Career":return"program";case "Course":return"course"}};a.getDatePlusDays=function(a,b){return a?new Date(a.getTime()+864E5*b):null};a.submitStep=function(b){a.step.submitting=!0;c.submitStep(b).then(function(){a.step.submitting=!1;$("html,body").animate({scrollTop:0})})};a.getCacheBustingUrl=function(a){return a&&a+"?"+g};a.currentUserName=f.getFullName();f.get("email")&&(a.currentUserEmail=f.get("email").address);a.currentUserNickname=f.get("nickname");a.step={};a.getCurrentStep=c.getCurrentStep;
a.getCurrentFlowSteps=c.getCurrentFlowSteps;a.$on("$destroy",function(){m.onbeforeunload=void 0});m.onbeforeunload=function(){a.flow.abandonedCheckout&&(a.flow.segmentProps.end_time=new Date,a.flow.segmentProps.time_on_page_sec=(a.flow.segmentProps.end_time-a.flow.segmentProps.start_time)/1E3,analytics.track("Abandoned Checkout Step",a.flow.segmentProps))}}]);angular.module("page.payment",["core"]).config(["$routeProvider",function(a){a.when("/:stepNumber",{}).otherwise({redirectTo:"/0"})}]);angular.module("payment").directive("dateExp",function(){return{require:"ngModel",link:function(a,c,b,d){d.$parsers.push(function(a){if(void 0==a)return"";var b;/^\d\d\s\/$/.test(a)?b=a.substr(0,1):(b=a.replace(/[^0-9]/g,""),4<b.length&&(b=b.substr(0,4)),/^\d{2,4}$/.test(b)&&(b=b.substr(0,2)+" / "+b.substr(2,b.length-2)));b!=a&&(d.$setViewValue(b),d.$render());return b})}}});angular.module("payment").constant("DAY_IN_MILLISECONDS",864E5).factory("fullClassExperienceFlowDefinition",["$q","stripeCreditCard","validator","api","currentUser","products","customers","couponCode","users","$timeout","$log","nodes","DAY_IN_MILLISECONDS","userTrials","googleTagManager","universalAnalytics","$window",function(a,c,b,d,e,f,g,h,i,j,m,l,k,p,q,n,r){function u(a){return a?(a^16*Math.random()>>a/4).toString(16):([1E7]+-1E3+-4E3+-8E3+-1E11).replace(/[018]/g,u)}function w(a){return"card_declined"===
a.code?"Your card was declined. Please try again. If this issue persists, please contact payment-support@udacity.com.":"processing_error"===a.code?"An error occurred while processing your card. Please try again.If this issue persists, please contact payment-support@udacity.com.":a.code?a.message||a.error:"An error occurred while processing your card. Please check your information and try again. If this issue persists, please contact payment-support@udacity.com."}function t(a){return a&&_.map(a._course_groups,
function(a){return a.course_ref&&a.course_ref.key})}function s(b,c){var e={product_keys:c.productKeys};b&&b.id&&(e.stripe_token=b.id);c.coupon&&c.coupon.valid&&(e.coupon_id=c.coupon.key);e=d.post("/customers/me/purchases",e).then(function(a){return a.data.purchase},function(b){c.generalPaymentFormError=b.data.error?w(b.data):"We are having difficulties processing your request. If this error keeps appearing please contact payment-support@udacity.com.";b.data.was_charged&&alert("Error - your card was charged successfully but an error occured.Please contact us at payment-support@udacity.com immediately and we'll get this sorted out. Your spot is reserved. Please do NOT pay again.");
return a.reject(b)});e.then(function(){return c.course.isNanodegree&&c.course.shouldSeeOnboarding?i.setOnboardingFlag("me",c.course.key):a.when()});return e}function x(a,b,c,d,f){var g={step:1,cc_prefilled:!!a.customerCard,sku:null,product_key:null,name:null,price:null,currency:"USD",trial_period_days:null,coupon:null,coupon_valid:!1,coupon_description:null,coupon_redeem_by:null,discount_amount:0,discount_type:null,enrolled_nanodegrees:e.getEnrolledNanodegreeKeys().toString(),enrolled_courses:e.getEnrolledCourseKeys().toString(),
price_before_discount:null,funnel_id:y};a.product&&(g.product_key=a.product.key,g.name=a.product.name,g.price=a.product.amount/100,g.currency=a.product.currency,g.trial_period_days=a.product.trial_period_days,g.price_before_discount=g.price,g.value=g.price);a.course&&(g.category="Degree"===a.course.courseType?"nanodegree":"Career"===a.course.courseType?"career_advisory":"course",g.sku=a.course.key,g.label=g.sku);a.coupon&&a.coupon.key&&(g.coupon=a.coupon.key,g.coupon_description=a.coupon.description||
null,g.coupon_redeem_by=a.coupon.redeem_by||null,g.discount_type=a.coupon.duration||null,a.coupon.valid&&(g.coupon_valid=a.coupon.valid,g.discount_amount=g.price_before_discount?(a.product.amount-a.coupon.discounted_price)/100:0,g.price=a.coupon.discounted_price/100));"Added Product"===c?g.quantity=1:"Completed Order"===c&&(g=_.extend(g,{orderId:b,total:g.price,revenue:g.price,shipping:0,tax:0,discount:g.discount_amount,coupon:g.coupon,currency:g.currency,products:[{id:g.product_key,sku:g.sku,name:g.name,
price:g.price,quantity:1,category:g.category}]}));d&&(g.start_time=d);f&&(g.end_time=new Date,g.time_on_page_sec=(g.end_time-g.start_time)/1E3);return g}var y=u();return[{label:"Checkout",view:"/templates/angular/payment/payment_form.html",onStart:function(b,e){e.funnel_id=y;var i=[f.get(e.productKeys[0]),g.get("me")];if(h){var j=d.get("/coupons/"+h+"/compute_discount",{product_key:e.productKeys[0]}),j=j.then(function(a){e.coupon=a.data.coupon;a.data.coupon.valid&&(e.coupon.discounted_price=a.data.result)},
function(){e.coupon={valid:!1};return null});i.push(j)}a.all(i).then(function(a){e.product=a[0];var b=_.first(e.product._course_groups).course_ref.key;l.get(b).then(function(a){var b=a.catalog_entry._image?a.catalog_entry._image.serving_url:null,c=a.overview_details?a.overview_details.should_see_onboarding:!1;e.confirmationStepLabel={Career:"Start program",Degree:"Start Nanodegree",Course:"Start course"}[a.getCourseType()];e.course={key:a.key,imageUrl:b,isNanodegree:a.isNanodegree(),isCareer:a.isCareer(),
courseType:a.getCourseType(),shouldSeeOnboarding:c};q.pushTrackingEvent("checkoutStart",{productName:e.product.name,productCurrency:e.product.currency,productPrice:e.product.amount,orderNodeKey:e.course.key,isNanodegree:a.isNanodegree,isCareer:a.isCareer,getCourseType:a.getCourseType});a=x(e,null,"Viewed Checkout Step");e.segmentProps=a;e.segmentProps.start_time=new Date;analytics.track("Viewed Checkout Step",a,r.segmentPageProperties);analytics.track("Added Product",a,r.segmentPageProperties)});
b=e.product.trial_period_days;e.inTrialPeriod=0<b;e.trialPeriodStarts=new Date(Date.now()+b*k);var b=new Date(e.trialPeriodStarts),d=b.getMonth();b.setMonth(b.getMonth()+1);b.getMonth()!=(d+1)%12&&b.setDate(0);e.secondPaymentDue=b;e.customer=a[1];e.customerCard=c.getDefaultCard(e.customer);e.cardIsLoaded=!0;e.uriEncodedCourseKey=window.encodeURIComponent(t(e.product))})},onJump:function(a){return a.paymentConfirmed?!1:!a.paymentConfirmed},onSubmit:function(b,d){if(d.customerCard)return s(!1,d);var e=
b.creditCard||{},f=c.checkValidity(e);d.creditCardErrors=f.errorText;if(!f.valid)return!1;f=(d.product&&d.product.name)+", "+t(d.product);return c.createPaymentToken(f,e).then(function(a){return s(a,d)},function(b){d.generalPaymentFormError=w(b.error);return a.reject(b)})}},{getLabel:function(a){return a.confirmationStepLabel},view:"/templates/angular/payment/payment_confirmation.html",onStart:function(a,b){b.paymentConfirmed=!0;b.payment=a;var c=b.product&&(b.product.amount||0)/100,d=null;try{d=
_.find(a.customer.subscriptions,function(a){return a.plan.id===b.product.key}).id}catch(e){m.error("Failed to get Transaction Id",e)}n.trackTransaction({transactionId:d,transactionTotal:c,transactionAffiliation:"Udacity",transactionProducts:[{name:b.product&&b.product.name||"Missing product name, FILL IN IN CONTENT EDITOR ASAP FOR "+_.first(t(b.product)),sku:"DD44",price:c,quantity:1,category:_.first(t(b.product))}]});q.pushTrackingEvent("CompletedOrder",{transactionVal:c,transactionOrderId:d,transactionPromoCode:h,
orderNodeKey:b.course.key,isNanodegree:b.course.isNanodegree});b.abandonedCheckout=!1;analytics.track("Completed Checkout Step",x(b,d,"Completed Checkout Step",b.segmentProps.start_time,!0),r.segmentPageProperties);analytics.track("Completed Order",x(b,d,"Completed Order",b.segmentProps.start_time,!0),r.segmentPageProperties)}}]}]);angular.module("payment").directive("numbersOnly",function(){return{require:"ngModel",link:function(a,c,b,d){d.$parsers.push(function(a){if(void 0==a)return"";var b=a.replace(/[^0-9]/g,"");b!=a&&(d.$setViewValue(b),d.$render());return b})}}});angular.module("payment").factory("paymentFlow",["$q","paymentState","$location",function(a,c,b){return{begin:function(a){a=c.startFlow(a);if(a.onStart)a.onStart(null,c.getCurrentFlowData());b.path("/"+c.getCurrentStep().number.toString())},submitStep:function(d){var e=a.defer();(d=c.getCurrentStep().onSubmit(d,c.getCurrentFlowData()))?a.when(d).then(function(a){var d=c.goToNextStep();if(d.onStart)d.onStart(a,c.getCurrentFlowData());e.resolve();b.path("/"+d.number.toString())},function(){e.resolve()}):
e.resolve();return e.promise},goToStep:function(a){a=window.parseInt(a,10);if(a!==c.getCurrentStep().number&&(a=c.getStepNumber(a),!a.onJump||a.onJump(c.getCurrentFlowData()))){if(a.onStart)a.onStart(null,c.getCurrentFlowData());b.path("/"+c.goToNextStep().number.toString())}},isInFlow:c.isInFlow,getCurrentStep:c.getCurrentStep,getCurrentFlowSteps:c.getCurrentFlowSteps,getCurrentFlowData:c.getCurrentFlowData}}]);angular.module("payment").factory("paymentFlowDefinitions",["fullClassExperienceFlowDefinition",function(a){var c={"default":a};_.each(c,function(a){_.each(a,function(a,b){a.number=b})});return{get:function(){return c["default"]}}}]);angular.module("payment").controller("paymentForm",["$scope","$filter","api","customers",function(a,c,b,d){a.sendVerificationEmail=function(){b.post("/users/send_verification_email").success(function(){a.resentVerificationEmail=!0}).error(function(a){console.error("Something went wrong",a)})};d.isEligibleForCurrencyPriceMatch("me").then(function(b){a.isEligibleForCurrencyPriceMatch=b});a.refresh=function(){window.location.reload()};var e=a.flow;a.trialPeriod=function(a){if(!e.course)return"";var b=
e.course.isNanodegree||e.course.isCareer?"1 Week":"2 Weeks";return a?b.toLowerCase():b};a.firstPaymentDescription=function(){return!e.product?"":a.isProductNotLaunched(e.product)?"You will not be charged until the "+a.getDescStringByCourseType(e.course.courseType)+" launches. We will send you a reminder when the "+a.getDescStringByCourseType(e.course.courseType)+" starts.":"You won't be charged until "+c("date")(e.trialPeriodStarts,"MMMM d, yyyy")};a.isPaymentInfoEmpty=function(){if(a.flow.customerCard)return!1;
var b=a.flow.creditCard;return!b?!0:_.isEmpty(b.nameOnCard)||_.isEmpty(b.number)||!b.expiryMonth||!b.expiryYear||_.isEmpty(b.cvc)};a.isCourse=function(){return e.course&&!e.course.isNanodegree};a.isDataReady=function(){return!!e.course&&!!e.product}}]);angular.module("payment").factory("paymentState",["paymentFlowDefinitions",function(a){var c=null,b=null,d={};return{startFlow:function(e){c=a.get(e);b=c[0];d={productKeys:e||[]};return b},getCurrentStep:function(){return b},getStepNumber:function(a){return c[a]},isInFlow:function(){return!!c},getCurrentFlowSteps:function(){return c},getCurrentFlowData:function(){return d},setCurrentStepNumber:function(a){return b=c[a]},goToNextStep:function(){return b=c[b.number+1]}}}]);angular.module("payment").directive("priceMatchNotice",[function(){return{restrict:"EA",scope:{title:"=",eligible:"="},template:'<div class="row">\t<div class="col-xs-6 col-sm-8 col-sm-offset-2 col-md-4 col-md-offset-4 alert alert-success text-center" data-ng-if="eligible">\t\tYou are eligible for a currency adjustment refund.\t\t<p><small class="faded"><a data-ng-click="showCurrencyPriceMatchModal()">Click here for more information.</a></small></p>\t</div></div><div class="ng-cloak" data-modal-dialog="currencyPriceMatch">\t<div class="modal-header">\t\tInternational Refund\t</div>\t<div class="modal-body">\t\t<div class="ng-cloak modal-body">\t\t\t<p>"We recently launched a localized pricing model for our students in India. Going forward, residents of India are eligible for a subscription adjustment that will more closely reflect the INR price point. </p>\t\t\t<p>Please email <a href="mailto:payment-support@udacity.com?subject=INR Price Match Discount">payment-support@udacity.com</a>\t\t\tduring your 7-day Nanodegree free trial to have this adjustment made!</p>\t\t\t<p>Thank you!</p>\t\t</div>\t</div></div> ',
controller:["$scope","modal",function(a,c){a.showCurrencyPriceMatchModal=function(){c.activate("currencyPriceMatch")}}]}}]);angular.module("payment").factory("stripeCreditCard",["$http","$q","$rootScope","validator",function(a,c,b,d){var e=window.Stripe,f=!!e,g={createPaymentToken:function(a,d){var j=c.defer(),m=g.checkValidity(d);m.valid?f&&e.createToken({subject:a,name:d.nameOnCard,number:d.number,cvc:d.cvc,exp_month:d.expiryMonth,exp_year:d.expiryYear},function(a,c){b.$apply(function(){c.error?j.reject(c):j.resolve(c)})}):j.reject(m);return j.promise},checkValidity:function(a){return d.check({nameOnCard:{pretty:"cardholder's name",
...
alt:"employees discussing pedagogy"}];a.showFullImage=function(a){c.activate("galleryImage",a)}}]);angular.module("staticPages").directive("uTooltip",function(){return{link:function(a,c,b){$(c).tooltip({title:'<div class="tooltip-content">'+b.uTooltip+"</div>",html:!0,placement:"bottom"})}}}).controller("nanodegreePageController",["$scope","modal","$timeout","ndTiles","$location",function(a,c,b,d,e){-1!==(URI.parse(e.absUrl()).query||"").indexOf("utm_campaign=20151230_EMAIL_ACDOTIN_25OFF")&&c.activate("welcomeIndia");b(function(){a.playVideo=function(b,d){d||(d=b);var e=function(){a.$parent.$broadcast("pauseVideo",
b)};c.activate(d).then(e,e);a.$parent.$broadcast("playVideo",b)}});a.loading=!0;d.getInfo().then(function(b){a.loading=!1;a.nanodegrees=b});a.mainPageSocial={twitter:{text:"Get a job I love with a credential that fits into my life? Yes. Udacity Nanodegree programs, built by industry.",url:"https://www.udacity.com/nanodegree"},facebook:{name:"Introducing Udacity Nanodegree Credentials",link:"https://www.udacity.com/nanodegree",caption:"Industry credentials for today's jobs in tech",description:"Stand out with tech skills that matter. Learn from the very best in industry. Study smart without leaving your current job."},
gplus:{link:"https://www.udacity.com/nanodegree"}}}]);angular.module("staticPages").factory("ndTiles",["nodes",function(a){var c="nd000 nd001 nd002 nd003 nd004 nd006 nd007 nd801 nd009 nd802 nd011 nd010".split(" ");return{getInfo:function(){return a.batchGet(c,{depth:0,fresh:!0,projection:"catalog"}).then(function(a){_.map(a,function(a){return a.title?(name=a.title.replace(" Nanodegree",""),_.extend(a,{name:name})):a});return a})}}}]);angular.module("staticPages").controller("organizationPage",["$scope","validator","users",function(a,c,b){a.submitted=!1;a.errorList=null;a.postError=!1;a.isWorking=!1;a.showNoteField=!1;a.businessPartners=[{source:"//s3.amazonaws.com/content.udacity-data.com/site-svgs/google_svg.svg",text:"Training developers globally, including Google employees, in Android, web, and cloud development techniques and best practices.",alt:"Google"},{source:"//s3.amazonaws.com/content.udacity-data.com/site-svgs/att_svg.svg",
...
g.pending[e]&&g.pending[e].readyNotification(g.run);if(null!==b)g.pending=_.without(g.pending,b),b["finally"](g.run),g.running.push(b),c(b.start);else break}}};return g};d=a.make;return a}]);angular.module("user").factory("currentUser",["$log","$q","$rootScope","users","basicUser","geolocation","nodes","nodeUtility","userUtility","cohortsGateway","universalAnalytics","publicCatalog",function(a,c,b,d,e,f,g,h,i,j,m,l){var k=angular.copy(e),p=null,q=null,n={get:function(a){return k[a]},getUser:function(){return k},getFullName:function(){return i.getFullName(k)},fetchIsStaff:function(){return n.fetchFullUser().then(function(a){if(a._memberships)return 0<_.filter(a._memberships,function(a){return-1<
a.group_ref.key.indexOf("staff")}).length})},getEnrolledCourses:function(a){a=a||["enrolled"];return _.chain(n.get("_enrollments")).filter(function(b){return b&&_.contains(a,b.state)}).pluck("node_key").value()},getEnrolledCourseKeys:function(a){a=a||["enrolled"];return _.filter(n.getEnrolledCourses(a),function(a){return 0!==a.indexOf("nd")})},getEnrolledNanodegreeKeys:function(a){a=a||["enrolled"];return _.filter(n.getEnrolledCourses(a),function(a){return 0===a.indexOf("nd")})},getEnrolledCareerKeys:function(a){a=
a||["enrolled"];return _.filter(n.getEnrolledCourses(a),function(a){return 0===a.indexOf("ca")})},fetchGraduatedNanodegreeKeys:function(){return n.fetchCertificates().then(function(a){return _.chain(a).map(function(a){return a.node_ref.key}).filter(function(a){return 0===a.indexOf("nd")}).value()})},fetchGraduatedCourseKeys:function(){return n.fetchCertificates().then(function(a){return _.map(a,function(a){return a.node_ref.key})})},fetchGraduatedCareerKeys:function(){return n.fetchCertificates().then(function(a){return _.chain(a).map(function(a){return a.node_ref.key}).filter(function(a){return 0===
a.indexOf("ca")}).value()})},fetchEnrolledAndGraduatedCourseKeys:function(){return n.fetchFullUser().then(function(){return n.fetchGraduatedCourseKeys()}).then(function(a){return _.union(n.getEnrolledCourses(),a)})},fetchInferredCreatedAt:function(){return n.fetchFullUser().then(function(a){var b,a=_.chain(a._memberships).pluck("creation_time").compact().map(function(a){return new Date(a)}).value();a.length&&(b=_.min(a).toISOString());return b})},fetchEnrolledAndGraduatedNanodegreeKeys:function(){return n.fetchGraduatedNanodegreeKeys().then(function(a){return _.union(n.getEnrolledNanodegreeKeys(),
a)})},fetchEnrolledAndGraduatedCareerKeys:function(){return n.fetchGraduatedCareerKeys().then(function(a){return _.union(n.getEnrolledCareerKeys(),a)})},getGroupKeys:function(){return _.chain(n.get("_memberships")).filter(function(a){return a.group_ref&&"Group"===a.group_ref.ref}).map(function(a){return a.group_ref.key}).value()},fetchStudyGroups:function(){return d.getStudyGroups("me")},fetchActiveExperiments:function(a){return d.fetchActiveExperiments("me",a)},getEmail:function(){return i.getEmail(k)},
setUser:function(a){angular.copy(angular.copy(a),k)},fetchFullUser:function(a){var b=a?{cacheBust:Math.random()}:{};if(a)return d.get("me",b).then(function(a){n.setUser(a);return a});p||(p=d.get("me",b).then(function(a){n.setUser(a);return a}));return p},fetchCertificates:function(){return k._certificates?c.when(k._certificates):d.getCertificates("me").then(function(a){return k._certificates=a})},checkNewHomeBanner:function(){return d.checkNewHomeBanner("me")},setNewHomeBanner:function(){return d.setNewHomeBanner("me")},
setFinalStepsEmailSent:function(a){return d.setFinalStepsEmailSent("me",a)},sendFinalStepsEmail:function(a,b){return d.sendFinalStepsEmail("me",a,b)},setFiftyBackBannerFlag:function(a){return d.setFiftyBackBannerFlag("me",a)},fetchCohorts:function(){q||(q=n.fetchFullUser().then(function(a){return c.all(_.map(a._cohort_keys,function(a){return j.get(a)}))},function(b){a.error(b)}));return q},userIsLoggedIn:function(){return k._registered},isEnrolledIn:function(a){return _.some(k._enrollments,function(b){return b.node_key===
a&&"enrolled"===b.state})},isEnrolledForCreditIn:function(a){var b=["151101088","150207572","151148782","151059885","151254701"];return void 0!==_.find(k._enrollments,function(c){var d=_.pluck(c.group_refs,"key");return c.node_key===a&&"enrolled"===c.state&&0<_.intersection(d,b).length})},isCourseEmailSubscribed:function(a){return 0<_.filter(k._enrollments,function(b){return b.node_key===a&&b.email_ok}).length},enroll:function(a,c,e){c=c||"enrolled";return d.setEnrollments("me",[a],c).then(function(a){k._enrollments=
k._enrollments||[];_.each(a,function(a){var b=_.find(k._enrollments,function(b){return b.node_key===a.node_key});b?k._enrollments[_.indexOf(k._enrollments,b)]=a:k._enrollments.push(a)});if(1===a.length){b.$broadcast("modifiedEnrollment",a);var d=c.slice(0,1).toUpperCase()+c.slice(1,-2),d=m.trackEvent({category:"Enrollment",action:d,label:a[0].node_key});return e?d.then(function(){return a},function(){console.warn("Failed to push enrollment to Universal Analytics");return a}):a}return a})},changeEnrollmentSettings:function(a,
b){return d.changeEnrollmentSettings(a,"me",b)},getEnrollmentsWithState:function(a){return _.filter(k._enrollments,function(b){return b.state===a})},getTags:function(){return d.getTags("/users/me/tags")},setTag:function(a,b){var e=n.get("tags"),a=a.toLowerCase(),b=b.toLowerCase();if(_.findWhere(e,{namespace:a,tag:b}))return c.when(e);var f=e.concat([{namespace:a,tag:b}]);k.tags=f;return d.put({key:n.get("key"),tags:f}).catch(function(a){k.tags=e;return c.reject(a)})},removeTag:function(a,b){var e=
n.get("tags");if(!_.findWhere(e,{namespace:a,tag:b}))return c.when(e);var f=_.reject(e,function(c){return c.namespace===a&&c.tag===b});k.tags=f;return d.put({key:n.get("key"),tags:f}).catch(function(a){k.tags=e;return c.reject(a)})},isNanodegreeStudent:function(){return 0<_.size(n.getEnrolledNanodegreeKeys(["enrolled","unenrolled"]))},checkIfProfilesEnabled:function(){var a=n.getEnrolledNanodegreeKeys(["enrolled","pre-enrolled","unenrolled"]);return 0===_.size(a)?c.when(!1):l.fetchCatalogAsMapOfNodeKeysToNode().then(function(b){return _.some(a,
function(a){return b[a]&&b[a].enables_profiles})})},isUserEnrolledInCourseThroughND:function(a){return a.related_degree_keys||a.catalog_entry&&a.catalog_entry.related_degree_keys?n.fetchEnrolledAndGraduatedNanodegreeKeys().then(function(b){return 0<_.size(_.intersection(a.related_degree_keys?a.related_degree_keys:a.catalog_entry.related_degree_keys,b))}):-1<a.key.indexOf("-nd")?c.when(!0):c.when(!1)},fetchIsReviewer:function(){return d.fetchIsUserReviewer(k.key)},discoverAndSetCurrency:function(){if(!n.get("currency"))return f.get().then(function(a){d.updateCountry("me",
a.data.country)})},copyProgressToUdacityTwoDotO:function(){return d.copyProgressToUdacityTwoDotO("me")}};return n}]);angular.module("user").directive("userMenu",["currentUser","session",function(a,c){return{restrict:"A",link:function(b){b.isLoggedIn=a.userIsLoggedIn;b.user=a.getUser;b.isNanodegreeStudent=a.isNanodegreeStudent();b.isProfilesEnabled=!1;a.checkIfProfilesEnabled().then(function(a){b.isProfilesEnabled=a});var d=[/\/account/,/\/me/,/\/checkout/];b.logOut=function(){analytics.track("Signed Out",{category:"Sign Out"},b.segmentPageProperties,function(){c.terminate().then(function(){analytics.reset();var a=
window.location.pathname;_.some(d,function(b){return b.test(a)})?window.location="/":window.location.reload()})})}}}}]);angular.module("user").factory("userUtility",[function(){return{getFullName:function(a){if(a)return a.first_name&&a.last_name?a.first_name+" "+a.last_name:a.first_name||a.last_name},getEmail:function(a){return a.email&&a.email.address}}}]);angular.module("utility").directive("animatedToggle",function(){return{restrict:"A",link:function(a,c,b){b.toggleTarget||console.error("You must provide a toggle-target with a selector for the child element to toggle");b.toggler||console.error("You must provide an element to trigger the toggle");var d=c.find(b.toggleTarget),a=c.find(b.toggler);d||console.error("Element to toggle ("+b.toggleTarget+") not found!");a||console.error("Element to trigger toggle ("+b.toggler+") not found!");$(a).on("click",
function(){$(d).toggle("fast")});window.location.hash.substring(1)===a.attr("id")?$(d).show():$(d).hide()}}});angular.module("utility").service("applicationEnvironment",["$location",function(a){var c=/(abcinth)|(localhost)/;return{isInDevelopment:function(){return c.test(a.absUrl())}}}]);angular.module("utility").factory("callbackList",function(){return{create:function(){var a=[],c,b=!1,d={add:function(e,f,g){a.push(e);b&&f?(e(c),g&&d.remove(e)):g&&(e._removeOnFire=!0);return d},remove:function(b){for(var c=a.length,g=0;g<c;g++)a[g]===b&&(a.splice(g,1),g-=1);return d},fire:function(e){var f,g=_.clone(a),h=g.length;for(f=0;f<h;f++)try{g[f](e)}catch(i){console.error("Exception encountered in callback:"+i.message),console.error(g[f])}c=e;b=!0;for(f=0;f<a.length;f++)!0===a[f]._removeOnFire&&
...
i=null,j=0,m=f.length;j<m;j++){i=f[j].match(/([^=]+)=/i);if(angular.isArray(i))try{g=decodeURIComponent(i[1]),h=!1===c?f[j].substring(i[1].length+1):decodeURIComponent(f[j].substring(i[1].length+1))}catch(l){}else g=decodeURIComponent(f[j]),h="";e[g]=h}return e};return{exists:function(a){if(!angular.isString(a)||""===a)throw new TypeError("$cookieJar.exists(): Cookie name must be a non-empty string.");return c(document.cookie,!0).hasOwnProperty(a)},get:function(a,d){var d=angular.isObject(d)?d:{},
...
if(null!==f){var g=+f[1],h=+f[3],f=new Date(h,g-1,+f[2]);return(""+f.getFullYear()).slice(-2)===(h+"").slice(-2)&&f.getMonth()===g-1?a:e}return e}};return{check:function(a){var d,e={valid:!0,errorText:{}},f,g;for(g in a)if(a.hasOwnProperty(g)){d=a[g];for(var h=0;h<d.validators.length;h++)if(f=d.validators[h],"undefined"===typeof f.disabled||!1===f.disabled)if("function"===typeof c[f.type]){var i=d.value,j=d.pretty||g,m=c[f.type](i,j,f.args||[]);m.valid?f.hasOwnProperty("chained")&&(m={},m[g]={pretty:j,
value:i,validators:f.chained},f=this.check(m),!1===f.valid&&(e.valid=!1,e.errorText[g]=e.errorText[g]||[],e.errorText[g]=e.errorText[g].concat(f.errorText[g]))):(e.valid=!1,e.errorText[g]=e.errorText[g]||[],e.errorText[g].push("undefined"===typeof f.failMsg?m.errorText.charAt(0).toUpperCase()+m.errorText.slice(1):f.failMsg))}else console.warn("You tried to use a validator that doesn't exist! The name you tried to use was: "+f.type)}return e}}}]);angular.module("viewer").controller("contentGate",["$scope","modal","currentUser","$location",function(a,c,b,d){a.signUpAndReturnToCourse=function(a){window.location.href="/account/auth#!/signup?next=%2Fcourse%2Fviewer%23!"+encodeURIComponent(d.path())+"&enroll="+encodeURIComponent(a)};a.signInAndReturnToCourse=function(a){window.location.href="/account/auth#!/signin?next=%2Fcourse%2Fviewer%23!"+encodeURIComponent(d.path())+"&enroll="+encodeURIComponent(a)};a.enrollInCourse=function(d){a.enrollRequestPending=
...
c,b,d,e,f,g,h,i,j,m,l,k,p,q,n,r,u,w,t,s,x,y,H,I,E,F,G){function C(){var b;b=_.without(_.union(m.general,m[v]),null,void 0,"");a.viewerLoadingMessage=b[_.random(b.length-1)];a.isViewerLoading=!0}function A(a,b,c){if(a){var d=q[a.model];if(d){if(!c||d.extension)return"/templates/angular/viewer/partials/"+d.prefix+b+"?"+j}else console.error("Unrecognized morsel type "+a.model)}else if(!c)return"/templates/angular/viewer/partials/loading"+b+"?"+j}function B(a){f.cancel(z);("Video"===a.model||"SketchVideo"===
a.model)&&h.addEventListener("videoPlay",function(){f.cancel(z);z=f(D(a.key),1E3*g,!1)},{removeAfterFired:!0})}function D(a){return function(){if(h.getCurrentTime()>=g){var b=h.getCurrentTime();h.emitEvent("videoPause",b);h.emitEvent("videoPlay",b+1);c.markMorselAsCompleted(a)}else z=f(D(a),500,!1)}}a.modalsUrl="/templates/viewer/partials/viewer_modals.html?"+j;a.getCoachServicesUrl=function(){if(a.course)return"/conditional/tutoring?course_id="+window.encodeURIComponent(a.course.key)+"&cache_bust="+
j};a.userIsLoggedIn=s.userIsLoggedIn();a.canEditNotes=y;a.course=null;a.lessons=null;a.lesson=null;a.morsels=null;a.morsel=null;a.tidbit=null;var v=b.path().match("/"+k.course+"([^/]+)"),i=b.path().match("/"+k.lesson+"([^/]+)"),v=v?v[1]:null,i=i?i[1]:null;a.courseKeyFromUrl=v;a.lessonKeyFromUrl=i;v||(window.location.href="/");u.addEventListener("sessionExpired",function(){window.location.href="/account/auth#!/signin?next=%2Fcourse%2Fviewer%23!"+encodeURIComponent(b.path())});w&&(window.location.href=
"/account/auth#!/signin?next=%2Fcourse%2Fviewer%23!"+encodeURIComponent(b.path()));c.addEventListener("lessonPermissionDenied",function(){window.location.href="/course/"+v});c.addEventListener("courseChange",function(a){if((!a.lessons||0===a.lessons.length)&&!a.lastLessonVisited&&!d.lessonKey)window.location.href="/course/"+v}).addEventListener("coursePermissionDenied",function(){d.lessonKey||(window.location.href="/course/"+v)});c.addEventListener("lessonChange",function(a){if(!a.morsels||0===a.morsels.length)window.location.href=
"/course/"+v});a.isViewerLoading=!0;c.addEventListener("courseChange",function(){p.loadLibraries(["codemirror","mathquill"]).then(function(){},function(){F.error("Classroom failed to load a required library");alert("Failed to load some required parts of the course! Please refresh the page. If this issue persists, please contact support@udacity.com. Thanks!")})}).addEventListener("morselChange",function(){a.isViewerLoading=!1});c.addEventListener("courseChange",function(b){a.course=b.course;a.lessons=
b.lessons;a.enrollment=_.find(s.get("_enrollments"),function(a){return a.node_key===b.course.key});a.premiumAccess=x.hasPermissionTo(a.course,"view-paid-supplemental-media")||a.enrollment&&(x.isCurrentEnrolled(a.enrollment,b.course)||x.isGraduated(a.enrollment,b.course));E.set({isPaid:a.premiumAccess?"yes":"no"})},{replayLast:!0,removeOnDestructionOf:a}).addEventListener("lessonChange",function(b){a.lesson=b.lesson;a.morsels=b.morsels},{replayLast:!0,removeOnDestructionOf:a}).addEventListener("morselChange",
function(b){a.morsel=b;B(b)},{replayLast:!0}).addEventListener("tidbitChange",function(b){(a.tidbit=b.tidbit)&&B(b.tidbit)},{replayLast:!0});c.addEventListener("pendingLessonChange",function(){a.morsels=null;a.morsel=null;a.tidbit=null});a.$on("$routeChangeSuccess",function(b,f,g){if(b=!d.morselKey)if(!g||!f)b=!1;else{var h=g.pathParams,f=f.pathParams;if(_.keys(h).length===_.keys(f).length)b=!1;else{var i=!0;_.each(f,function(a,b){h[b]!==a&&(i=!1)});b=i}}b&&window.history.back();d.courseKey&&d.courseKey!==
v&&window.location.reload();g&&G.trackVirtualPageView();if((null===a.course||a.course.key!==d.courseKey)&&"none"!==d.courseKey)C(),c.addEventListener("courseChange",function(b){d.lessonKey||(b.lastLessonVisited?a.goToLesson(b.lastLessonVisited.key):a.goToLesson(b.lessons[0].key))},{removeAfterFired:!0}),c.changeCourse(d.courseKey);if((null===a.lesson||a.lesson.key!==d.lessonKey)&&d.lessonKey)C(),c.addEventListener("lessonChange",function(b){d.morselKey||(b.lastMorselVisited?a.goToMorsel(b.lastMorselVisited):
a.goToMorsel(b.morsels[0]))},{removeAfterFired:!0}),c.changeLesson(d.lessonKey);if((null===a.morsel||a.morsel.key!==d.morselKey)&&d.morselKey)e.logNodeVisit("morsel"),a.lesson&&a.lesson.key===d.lessonKey?c.changeMorsel(d.morselKey):c.addEventListener("lessonChange",function(){c.changeMorsel(d.morselKey)},{removeAfterFired:!0});if(null===a.tidbit||a.tidbit.key!==d.tidbitKey)d.tidbitKey&&e.logNodeVisit("tidbit"),a.morsel&&a.morsel.key===d.morselKey?c.changeTidbit(d.tidbitKey):c.addEventListener("morselChange",
function(){c.changeTidbit(d.tidbitKey)},{removeAfterFired:!0})});a.alert=function(a){alert(a)};a.log=function(a){console.log(a)};a.getCurrentMorselType=function(){return a.morsel?a.morsel.model:null};a.getStyleClassesForMorsel=function(b){var c="";if(a.morsel)if("Exercise"===b.model){if(b.completed||b.lecture_ref&&b.lecture_ref.completed)c+="is-viewed";if(b.key===(a.morsel.containingExercise?a.morsel.containingExercise.key:a.morsel.key))c+=" is-current"}else b.completed&&(c+="is-viewed"),a.morsel.key===
b.key&&(c+=" is-current");return c};a.getPlayerPartialForCurrentMorsel=function(){return A(a.morsel,"_player.html")};a.getPlayerExtensionPartialForCurrentMorsel=function(){return A(a.morsel,"_extension.html",!0)};a.getSidePanelPartialForCurrentMorsel=function(){return A(a.morsel,"_side_panel.html")};a.goToMorsel=function(e){(!e||!e.key)&&console.error("Cannot go to invalid morsel "+e);(!a.morsel||e.key!==a.morsel.key)&&f(function(){if("Exercise"===e.model){var a=e.lecture_ref||e.quiz_ref;c.announcePendingMorselChange(a);
b.path("/"+k.course+d.courseKey+"/"+k.lesson+d.lessonKey+"/"+k.exercise+e.key+"/"+k.morsel+a.key)}else c.announcePendingMorselChange(e),e.containingExercise?b.path("/"+k.course+d.courseKey+"/"+k.lesson+d.lessonKey+"/"+k.exercise+e.containingExercise.key+"/"+k.morsel+e.key):b.path("/"+k.course+d.courseKey+"/"+k.lesson+d.lessonKey+"/"+k.morsel+e.key)})};a.goToTidbit=function(c){!c&&a.tidbit?d.exerciseKey?b.path("/"+k.course+d.courseKey+"/"+k.lesson+d.lessonKey+"/"+k.exercise+d.exerciseKey+"/"+k.morsel+
d.morselKey):b.path("/"+k.course+d.courseKey+"/"+k.lesson+d.lessonKey+"/"+k.morsel+d.morselKey):(!a.tidbit||c.key!==a.tidbit.key)&&f(function(){b.path("/"+k.course+d.courseKey+"/"+k.lesson+d.lessonKey+"/"+k.exercise+c.containingExercise.key+"/"+k.morsel+c.containingExercise.quiz_ref.key+"/"+k.tidbit+c.key)})};a.goToNextMorsel=function(){var b=a.morsel,c=a.morsels.length;b.containingExercise&&(b=b.containingExercise);b=b.indexInLesson;b<c-1?a.goToMorsel(a.morsels[b+1]):(h.pauseVideo(),a.course?s.userIsLoggedIn()?
a.lessons&&a.lesson.key===a.lessons[a.lessons.length-1].key?(a.enrollment=_.find(s.get("_enrollments"),function(b){return b.node_key===a.course.key}),t.activate("endOfCourse",{courseIsComplete:a.course.catalog_entry.complete,courseKey:a.course.key,isPaidEnrolled:x.isCurrentEnrolled(a.enrollment,a.course),isGraduated:x.isGraduated(a.enrollment,a.course)})):t.activate("endOfLesson",{lessonKey:a.lesson.key,userIsLoggedIn:s.userIsLoggedIn()}):t.activate("endOfLessonSignInGate",{lessonKey:a.lesson.key,
userIsLoggedIn:s.userIsLoggedIn()}):t.activate("endOfIsolatedContent"))};a.goToPreviousMorsel=function(){var b=a.morsel;b.containingExercise&&(b=b.containingExercise);b=b.indexInLesson;0<b?a.goToMorsel(a.morsels[b-1]):console.error("Can't go to previous; at beginning of lesson")};a.goToLesson=function(e){if(!a.lesson||e!==a.lesson.key)c.announcePendingLessonChange(),f(function(){b.path("/"+k.course+d.courseKey+"/"+k.lesson+e)})};a.goToNextLesson=function(){if(a.lessons){var b=a.lessons.length,c=_.find(a.lessons,
function(b){return b.key===a.lesson.key}).indexInCourse;0<=c&&c<b-1?a.goToLesson(a.lessons[c+1].key):console.error("Can't go to next; at end of course!")}else console.error("Can't go to next lesson, we're not in a course!")};a.os=r.os(!1);a.isActiveMorsel=function(b){return a.morsel&&a.morsel.key===b};a.getProgressPercentage=function(){return 77};var z}]);angular.module("viewer").controller("viewerPlayer",["$scope","lesson","$timeout","$window","currentUser","userTrials",function(a,c,b,d,e,f){a.isPlayerLoading=!0;d=function(a){var b=["attempt-1","attempt-2","control"],a=_.reduce(("skip-button-test-"+a).split(""),function(a,b){return(37*a+b.charCodeAt(0))%1009},0);return b[a%b.length]};a.skipButtonTrial=e.userIsLoggedIn()?f.getOrEnrollManually("skip_button_trial",d,[e.get("key")]):"excluded";a.loadingComplete=function(c){c?b(function(){a.$apply(function(){a.isPlayerLoading=
...
!0,a=$.extend({},a),d.init(a))},{replayLast:!0,removeOnDestructionOf:b})},{removeOnDestructionOf:b,removeAfterFired:!0});c.addEventListener("tidbitChange",function(a){b.tidbitViewerMode=e.getTidbitViewerMode(a.tidbit)},{replayLast:!0,removeOnDestructionOf:b});var h=g.find(".code-mirror-holder"),i=g.find(".editor-file-tabs"),j=function(){h.outerHeight(g.height()-(b.files&&1<b.files.length?i.outerHeight():0))},m=_.debounce(j,750),l=_.throttle(j,200);$(window).on("resize",m);$(window).on("resize",l);
...
b.submitFeedbackClicked=!1;b.showFeedbackDialog=function(){b.submitFeedbackClicked=!0;b.morselTitle=b.morsel.title;"Video"===b.morsel.model?(g.pauseVideo(),b.feedbackOptions=j):b.feedbackOptions="ImageFormQuiz"===b.morsel.model||"ProgrammingQuiz"===b.morsel.model?m:l;d.activate("feedback")};var k=function(a){for(var a=_.pairs(a),b={},c=0;c<a.length;++c){var d=a[c][0],e=a[c][1];if(_.isObject(e))for(var e=_.pairs(k(e)),f=0;f<e.length;++f)b[d+"."+e[f][0]]=e[f][1];else void 0!==e&&null!==e&&(b[d]=e)}return b};
b.feedbackSubmitted=!1;b.submitFeedback=function(){b.feedbackSubmitted=!0;b.formValidationResults=a.check(b.form);if(b.formValidationResults.valid){var l=[{context:e.getCurrent(),data:{model:"SurveyResponse",survey_name:"LessonFeedbackVersion2"}}],j={reason:b.form.feedbackReason.value,explanation:b.form.feedbackExplanation.value,techDetect:f.getAllTech(),currentUrl:window.location.href};"Video"===b.morsel.model&&(j=_.extend(j,{youtube:{availablePlaybackRates:g.getAvailablePlaybackRates(),currentPlaybackRate:g.getCurrentPlaybackRate(),
currentTime:g.getCurrentTime()}}));j=k(j);l[0].data.items=_.map(_.pairs(j),function(a){return{name:a[0],content:a[1]}});c.send(l);d.confirm();b.form.feedbackReason.value=null;b.form.feedbackExplanation.value=null;d.activate("feedback-confirmation").then(function(){"Video"===b.morsel.model&&g.playVideo()})}}}]}}]);angular.module("viewer").factory("codeEditor",["mixin","codeMirror","libraryLoader",function(a,c,b){function d(a){return-1!==a.indexOf(".c")?"c":-1!==a.indexOf(".cpp")?"cpp":-1!==a.indexOf(".css")?"css":-1!==a.indexOf(".cu")?"cuda":-1!==a.indexOf(".e")?"specman":-1!==a.indexOf(".glsl")?"glsl":-1!==a.indexOf(".html")?"html":-1!==a.indexOf(".java")?"java":-1!==a.indexOf(".js")?"javascript":-1!==a.indexOf(".json")?"javascript":-1!==a.indexOf(".m")?"octave":-1!==a.indexOf(".ocl")?"ocl":-1!==a.indexOf(".py")?
...
setActiveLanguage:function(a){d=a;b.setOption("mode",e[a]);h.emitEvent("languageChanged",{to:a})},getActiveLanguage:function(){return d},setOptions:function(a){_.each(a,function(a,c){b.setOption(c,a)})},getContents:function(){return b?b.getValue():null},setContents:function(a){b.setValue(a)}};return a.eventEmission(h,["attached","languageChanged","contentChanged"])}]);angular.module("viewer").constant("GaTechCourseKeys","cs373 gt101 ud007 ud037 ud061 ud098 ud129 ud169 ud189 ud219 ud233 ud262 ud281 ud333 ud409 ud436 ud459 ud501 ud514 ud557 ud572 ud600 ud640 ud675 ud702 ud741 ud753 ud758 ud805 ud809 ud810 ud819 ud820 ud821 ud912 ud915 ud923 ud953 ud954 ud955 ud957".split(" ")).constant("NdCourseNames",{nd001:"Front-end Web Developer",nd000:"Intro to Programming",nd002:"Data Analyst",nd003:"iOS Developer",nd004:"Full Stack Web Developer",nd006:"Beginning iOS App Development",
nd007:"Tech Entrepreneur",nd801:"Android Developer"}).factory("FreeToNdCourseMap",["FreeToNdCourseList",function(a){return _.reduce(a,function(a,b){a[b.course_key]=b;return a},{})}]).constant("FreeToNdCourseList",[{course_key:"bio110",title:"Tales from the Genome",level:"beginner",nd:"nd001",weight:0},{course_key:"ca001",title:"Career Advisor: Front-End Web Developer",level:"intermediate",nd:"nd000",weight:5},{course_key:"cs046",title:"Intro to Java Programming",level:"beginner",nd:"nd000",weight:5},
{course_key:"cs101",title:"Intro to Computer Science",level:"beginner",nd:"nd000",weight:5},{course_key:"cs101-old",title:"Introduction to Computer Science (February 2012)",level:"beginner",nd:"nd000",weight:5},{course_key:"cs212",title:"Design of Computer Programs",level:"advanced",nd:"nd000",weight:5},{course_key:"cs215",title:"Intro to Algorithms",level:"intermediate",nd:"nd000",weight:5},{course_key:"cs222",title:"Differential Equations in Action",level:"intermediate",nd:0,weight:0},{course_key:"cs253",
title:"Web Development",level:"intermediate",nd:"nd004",weight:7},{course_key:"cs253",title:"Web Development",level:"intermediate",nd:"nd001",weight:6},{course_key:"cs255",title:"HTML5 Game Development",level:"intermediate",nd:"nd001",weight:6},{course_key:"cs256",title:"Mobile Web Development",level:"intermediate",nd:"nd001",weight:6},{course_key:"cs258",title:"Software Testing",level:"intermediate",nd:"nd004",weight:2},{course_key:"cs259",title:"Software Debugging",level:"intermediate",nd:"nd004",
weight:2},{course_key:"cs262",title:"Programming Languages",level:"intermediate",nd:0,weight:0},{course_key:"cs271",title:"Intro to Artificial Intelligence",level:"intermediate",nd:0,weight:0},{course_key:"cs271-old",title:"Introduction to Artificial Intelligence (aiclass)",level:"intermediate",nd:0,weight:0},{course_key:"cs291",title:"Interactive 3D Graphics",level:"intermediate",nd:0,weight:0},{course_key:"cs313",title:"Intro to Theoretical Computer Science",level:"intermediate",nd:0,weight:0},
{course_key:"cs344",title:"Intro to Parallel Programming",level:"advanced",nd:0,weight:0},{course_key:"cs348",title:"Functional Hardware Verification",level:"advanced",nd:0,weight:0},{course_key:"cs373",title:"Artificial Intelligence for Robotics",level:"advanced",nd:0,weight:0},{course_key:"cs373-old",title:"Artificial Intelligence for Robotics (February 2012)",level:"advanced",nd:0,weight:0},{course_key:"cs387",title:"Applied Cryptography",level:"advanced",nd:0,weight:0},{course_key:"design101",
title:"Intro to the Design of Everyday Things",level:"beginner",nd:0,weight:0},{course_key:"ep245",title:"How to Build a Startup",level:"intermediate",nd:"nd007",weight:3},{course_key:"gt101",title:"OMSCS Student Orientation",level:"beginner",nd:0,weight:0},{course_key:"io15",title:"Puzzle Hunt for Google I/O",level:"beginner",nd:0,weight:0},{course_key:"ma004",title:"Intro Algebra Review",level:"beginner",nd:0,weight:0},{course_key:"nd000",title:"Intro to Programming Nanodegree",level:"beginner",
nd:0,weight:0},{course_key:"nd001",title:"Front-End Web Developer Nanodegree",level:"intermediate",nd:0,weight:0},{course_key:"nd002",title:"Data Analyst Nanodegree",level:"intermediate",nd:0,weight:0},{course_key:"nd003",title:"iOS Developer Nanodegree",level:"intermediate",nd:0,weight:0},{course_key:"nd004",title:"Full Stack Web Developer Nanodegree",level:"intermediate",nd:0,weight:0},{course_key:"nd006",title:"Beginning iOS App Development",level:"beginner",nd:0,weight:0},{course_key:"nd007",
title:"Tech Entrepreneur Nanodegree",level:"intermediate",nd:0,weight:0},{course_key:"nd801",title:"Android Developer Nanodegree",level:"intermediate",nd:0,weight:0},{course_key:"ph100",title:"Intro to Physics",level:"beginner",nd:0,weight:0},{course_key:"ps001",title:"Intro to Psychology",level:"beginner",nd:0,weight:0},{course_key:"st095",title:"Statistics",level:"beginner",nd:"nd002",weight:5},{course_key:"st101",title:"Intro to Statistics",level:"beginner",nd:"nd002",weight:5},{course_key:"ud007",
title:"High Performance Computer Architecture",level:"advanced",nd:0,weight:0},{course_key:"ud008",title:"Udacity Orientation for Georgia Tech OMS Students",level:"beginner",nd:0,weight:0},{course_key:"ud015",title:"Object-Oriented JavaScript",level:"intermediate",nd:"nd001",weight:10},{course_key:"ud015-nd",title:"Object-Oriented JavaScript",level:"intermediate",nd:"nd001",weight:10},{course_key:"ud032",title:"Data Wrangling with MongoDB",level:"intermediate",nd:"nd002",weight:10},{course_key:"ud032-nd",
title:"Data Wrangling with MongoDB",level:"intermediate",nd:"nd002",weight:10},{course_key:"ud036",title:"Programming Foundations with Python",level:"beginner",nd:"nd004",weight:10},{course_key:"ud036-nd",title:"Programming Foundations with Python",level:"beginner",nd:"nd004",weight:10},{course_key:"ud037",title:"Knowledge-Based AI: Cognitive Systems",level:"advanced",nd:0,weight:0},{course_key:"ud060",title:"GTPE HPCA",level:0,nd:0,weight:0},{course_key:"ud061",title:"Computability, Complexity & Algorithms",
level:"advanced",nd:0,weight:0},{course_key:"ud065",title:"Computability, Complexity & Algorithms",level:0,nd:0,weight:0},{course_key:"ud072",title:"GTPE KBAI",level:0,nd:0,weight:0},{course_key:"ud088",title:"Full Stack Foundations",level:"intermediate",nd:"nd004",weight:10},{course_key:"ud088-nd",title:"Full Stack Foundations",level:"intermediate",nd:"nd004",weight:10},{course_key:"ud098",title:"GT - Refresher - Advanced OS",level:"intermediate",nd:0,weight:0},{course_key:"ud110",title:"Intro to AJAX",
level:"intermediate",nd:"nd001",weight:10},{course_key:"ud110-nd",title:"Intro to AJAX",level:"intermediate",nd:"nd001",weight:10},{course_key:"ud120",title:"Intro to Machine Learning",level:"intermediate",nd:"nd002",weight:10},{course_key:"ud120-nd",title:"Intro to Machine Learning",level:0,nd:"nd002",weight:10},{course_key:"ud129",title:"Machine Learning",level:0,nd:"nd002",weight:9},{course_key:"ud146",title:"Machine Learning",level:0,nd:"nd002",weight:9},{course_key:"ud156",title:"CS 6210: Advanced Operating Systems - Projects",
level:"intermediate",nd:0,weight:0},{course_key:"ud162",title:"Intro to Point & Click App Development",level:"beginner",nd:0,weight:0},{course_key:"ud175",title:"CS 6300: Software Development Process - Part 2 of 4",level:"intermediate",nd:0,weight:0},{course_key:"ud181",title:"GTPE Software Architecture & Design",level:0,nd:0,weight:0},{course_key:"ud186",title:"Advanced Operating Systems",level:0,nd:0,weight:0},{course_key:"ud189",title:"Advanced Operating Systems",level:"advanced",nd:0,weight:0},
{course_key:"ud196",title:"Computer Networking",level:0,nd:0,weight:0},{course_key:"ud197",title:"Intro to Relational Databases",level:"intermediate",nd:"nd004",weight:10},{course_key:"ud197",title:"Intro to Relational Databases",level:"intermediate",nd:"nd002",weight:7},{course_key:"ud197-nd",title:"Intro to Relational Databases",level:"intermediate",nd:"nd004",weight:10},{course_key:"ud201",title:"Intro to Inferential Statistics",level:"beginner",nd:"nd002",weight:6},{course_key:"ud216",title:"Software Development Processes",
level:0,nd:0,weight:0},{course_key:"ud219",title:"Georgia Tech - Self Assessment",level:0,nd:0,weight:0},{course_key:"ud233",title:"High Performance Computer Architecture",level:"advanced",nd:0,weight:0},{course_key:"ud245",title:"Intro to jQuery",level:"intermediate",nd:"nd001",weight:10},{course_key:"ud245-nd",title:"Intro to jQuery",level:"intermediate",nd:"nd001",weight:10},{course_key:"ud248",title:"Make Your Own 2048",level:"beginner",nd:"nd000",weight:7},{course_key:"ud248",title:"Make Your Own 2048",
level:"beginner",nd:"nd001",weight:6},{course_key:"ud257",title:"A/B Testing",level:"intermediate",nd:"nd002",weight:10},{course_key:"ud257",title:"A/B Testing",level:"intermediate",nd:"nd007",weight:3},{course_key:"ud257-nd",title:"A/B Testing",level:"intermediate",nd:"nd002",weight:10},{course_key:"ud262",title:"Machine Learning",level:"intermediate",nd:"nd002",weight:7},{course_key:"ud281",title:"High Performance Computing",level:"advanced",nd:0,weight:0},{course_key:"ud292",title:"HTML5 Canvas",
level:"beginner",nd:"nd001",weight:10},{course_key:"ud292-nd",title:"HTML5 Canvas",level:"beginner",nd:"nd001",weight:10},{course_key:"ud299",title:"Configuring Linux Web Servers",level:"advanced",nd:"nd004",weight:3},{course_key:"ud304",title:"Intro to HTML and CSS",level:"beginner",nd:"nd001",weight:10},{course_key:"ud304",title:"Intro to HTML and CSS",level:"beginner",nd:"nd004",weight:6},{course_key:"ud304-nd",title:"Intro to HTML and CSS",level:"beginner",nd:"nd001",weight:10},{course_key:"ud325",
title:"iOS Persistence and Core Data",level:"advanced",nd:"nd003",weight:10},{course_key:"ud325-nd",title:"iOS Persistence and Core Data",level:"advanced",nd:"nd003",weight:10},{course_key:"ud330",title:"Authentication & Authorization: OAuth",level:"intermediate",nd:"nd004",weight:10},{course_key:"ud359",title:"Intro to Data Science",level:"intermediate",nd:"nd002",weight:10},{course_key:"ud359-nd",title:"Intro to Data Science",level:"intermediate",nd:"nd002",weight:10},{course_key:"ud381",title:"Real-Time Analytics with Apache Storm",
level:"intermediate",nd:"nd002",weight:5},{course_key:"ud399",title:"Computer Networking 2 - Resource Control",level:"advanced",nd:0,weight:0},{course_key:"ud405",title:"2D Game Development with LibGDX",level:"intermediate",nd:0,weight:0},{course_key:"ud421",title:"iOS Networking with Swift",level:"advanced",nd:"nd003",weight:10},{course_key:"ud421-nd",title:"iOS Networking with Swift",level:"advanced",nd:"nd003",weight:10},{course_key:"ud432",title:"Computer Networking 3-Operations & Mgmt",level:"advanced",
nd:"nd004",weight:3},{course_key:"ud436",title:"Computer Networking",level:"intermediate",nd:"nd004",weight:3},{course_key:"ud459",title:"CS 6035 Intro to Information Security",level:0,nd:0,weight:0},{course_key:"ud494",title:"CS 6300: Software Development Process - Part 4",level:"intermediate",nd:0,weight:0},{course_key:"ud501",title:"Machine Learning for Trading",level:"intermediate",nd:"nd002",weight:3},{course_key:"ud507",title:"Data Visualization and D3.js",level:"intermediate",nd:"nd002",weight:10},
{course_key:"ud507-nd",title:"Data Visualization and D3.js",level:"intermediate",nd:"nd002",weight:10},{course_key:"ud509",title:"Product Design",level:"intermediate",nd:"nd007",weight:10},{course_key:"ud509-nd",title:"Product Design",level:"intermediate",nd:"nd007",weight:10},{course_key:"ud514",title:"Introduction to Health Informatics",level:"beginner",nd:0,weight:0},{course_key:"ud518",title:"App Monetization",level:"intermediate",nd:"nd007",weight:10},{course_key:"ud518-nd",title:"App Monetization",
level:"intermediate",nd:"nd007",weight:10},{course_key:"ud549",title:"JavaScript Testing",level:"intermediate",nd:"nd001",weight:10},{course_key:"ud549-nd",title:"JavaScript Testing",level:"intermediate",nd:"nd001",weight:10},{course_key:"ud552-nd",title:"Telling Computers What To Do",level:0,nd:"nd000",weight:10},{course_key:"ud557",title:"Computability, Complexity & Algorithms",level:"advanced",nd:0,weight:0},{course_key:"ud585",title:"Intro to iOS App Development with Swift",level:"intermediate",
nd:"nd003",weight:10},{course_key:"ud585-nd",title:"Intro to iOS App Development with Swift",level:"intermediate",nd:"nd003",weight:10},{course_key:"ud595",title:"Linux Command Line Basics",level:"beginner",nd:"nd004",weight:10},{course_key:"ud600",title:"Reinforcement Learning",level:"advanced",nd:0,weight:0},{course_key:"ud607",title:"How to Make an iOS App",level:"advanced",nd:"nd003",weight:10},{course_key:"ud607-nd",title:"How to Make an iOS App",level:"advanced",nd:"nd003",weight:10},{course_key:"ud611",
title:"Intro to DevOps",level:"advanced",nd:0,weight:0},{course_key:"ud617",title:"Intro to Hadoop and MapReduce",level:"intermediate",nd:"nd002",weight:8},{course_key:"ud629",title:"HPCA 3",level:0,nd:0,weight:0},{course_key:"ud645-nd",title:"The Power of Abstraction",level:"beginner",nd:"nd000",weight:10},{course_key:"ud651",title:"Data Analysis with R",level:"intermediate",nd:"nd002",weight:10},{course_key:"ud651-nd",title:"Data Analysis with R",level:"intermediate",nd:"nd002",weight:10},{course_key:"ud675",
title:"Machine Learning: Supervised Learning",level:"intermediate",nd:"nd002",weight:8},{course_key:"ud702",title:"Advanced Operating Systems",level:"advanced",nd:0,weight:0},{course_key:"ud710",title:"CS 6210: Advanced Operating Systems - Part 2 of 3",level:"advanced",nd:0,weight:0},{course_key:"ud712",title:"CS 6210: Advanced Operating Systems - Part 3 of 3",level:"advanced",nd:0,weight:0},{course_key:"ud716",title:"HPCA 2",level:0,nd:0,weight:0},{course_key:"ud719",title:"App Marketing",level:"intermediate",
nd:"nd007",weight:10},{course_key:"ud721-nd",title:"Webpages, Documents, and Structure",level:"beginner",nd:"nd000",weight:10},{course_key:"ud723",title:"Rapid Prototyping",level:"intermediate",nd:"nd007",weight:10},{course_key:"ud723-nd",title:"Rapid Prototyping",level:"intermediate",nd:"nd007",weight:10},{course_key:"ud741",title:"Machine Learning: Unsupervised Learning",level:"intermediate",nd:"nd002",weight:7},{course_key:"ud744-nd",title:"Working with the Full Stack",level:0,nd:"nd000",weight:10},
{course_key:"ud774",title:"Xcode Debugging",level:"advanced",nd:"nd003",weight:9},{course_key:"ud774-nd",title:"Xcode Debugging",level:"advanced",nd:"nd003",weight:9},{course_key:"ud775",title:"How to Use Git and GitHub",level:"beginner",nd:"nd000",weight:7},{course_key:"ud775-nd",title:"How to Use Git and GitHub",level:"beginner",nd:0,weight:0},{course_key:"ud788",title:"UIKit Fundamentals",level:"intermediate",nd:"nd003",weight:10},{course_key:"ud788-nd",title:"UIKit Fundamentals",level:"intermediate",
nd:"nd003",weight:10},{course_key:"ud788a-nd",title:"UIKit Fundamentals I",level:"intermediate",nd:"nd003",weight:10},{course_key:"ud788b-nd",title:"UIKit Fundamentals II",level:"intermediate",nd:"nd003",weight:10},{course_key:"ud802",title:"How to create <anything> in Android",level:"beginner",nd:"nd801",weight:8},{course_key:"ud804",title:"JavaScript Basics",level:"beginner",nd:"nd001",weight:10},{course_key:"ud804-nd",title:"JavaScript Basics",level:"beginner",nd:"nd001",weight:10},{course_key:"ud805",
title:"Software Development Process",level:"intermediate",nd:0,weight:0},{course_key:"ud806",title:"Get Your Startup Started",level:"intermediate",nd:"nd007",weight:3},{course_key:"ud808-nd",title:"How to Install Android Studio",level:"beginner",nd:0,weight:0},{course_key:"ud809",title:"Health Informatics in the Cloud",level:"beginner",nd:0,weight:0},{course_key:"ud810",title:"Introduction to Computer Vision",level:"intermediate",nd:0,weight:0},{course_key:"ud819",title:"Computer Networking",level:"advanced",
nd:0,weight:0},{course_key:"ud820",title:"Machine Learning: Reinforcement Learning",level:"intermediate",nd:"nd002",weight:7},{course_key:"ud821",title:"Software Architecture & Design",level:"intermediate",nd:0,weight:0},{course_key:"ud825",title:"Android Performance",level:"advanced",nd:"nd801",weight:7},{course_key:"ud827",title:"Intro to Descriptive Statistics",level:"beginner",nd:"nd002",weight:7},{course_key:"ud835",title:"Introduction to Health Informatics",level:0,nd:0,weight:0},{course_key:"ud837",
title:"Android Development for Beginners",level:"beginner",nd:"nd801",weight:9},{course_key:"ud843",title:"Intro to Android 2",level:0,nd:"nd801",weight:9},{course_key:"ud849",title:"UX Design for Mobile Developers",level:"beginner",nd:"nd003",weight:5},{course_key:"ud849",title:"UX Design for Mobile Developers",level:"beginner",nd:"nd801",weight:5},{course_key:"ud853",title:"Developing Android Apps",level:"intermediate",nd:"nd801",weight:10},{course_key:"ud853-nd",title:"Developing Android Apps: Fundamentals",
level:"intermediate",nd:"nd801",weight:10},{course_key:"ud855",title:"Advanced Android App Development",level:"advanced",nd:"nd801",weight:10},{course_key:"ud855-nd",title:"Advanced Android App Development",level:"advanced",nd:"nd801",weight:10},{course_key:"ud858",title:"Developing Scalable Apps in Python",level:"advanced",nd:"nd004",weight:10},{course_key:"ud859",title:"Developing Scalable Apps in Java",level:"advanced",nd:"nd004",weight:6},{course_key:"ud860",title:"Browser Rendering Optimization",
level:"advanced",nd:"nd001",weight:10},{course_key:"ud860-nd",title:"Browser Rendering Optimization",level:"advanced",nd:"nd001",weight:10},{course_key:"ud862",title:"Material Design for Android Developers",level:"intermediate",nd:"nd801",weight:10},{course_key:"ud862-nd",title:"Android Design for Developers",level:"intermediate",nd:"nd801",weight:10},{course_key:"ud867",title:"Gradle for Android and Java",level:"advanced",nd:"nd801",weight:10},{course_key:"ud867-nd",title:"Gradle for Android and Java",
level:"advanced",nd:"nd801",weight:10},{course_key:"ud875",title:"Android Ubiquitous Computing",level:"advanced",nd:"nd801",weight:10},{course_key:"ud875-nd",title:"Android Ubiquitous Computing",level:"advanced",nd:"nd801",weight:10},{course_key:"ud875A",title:"Android Wear Development",level:"advanced",nd:"nd801",weight:10},{course_key:"ud875B",title:"Android TV and Google Cast Development",level:"advanced",nd:"nd801",weight:10},{course_key:"ud875C",title:"Android Auto Development",level:"advanced",
nd:"nd801",weight:10},{course_key:"ud876",title:"Google Play Services",level:"advanced",nd:"nd801",weight:10},{course_key:"ud876-1",title:"Google Play Services: Location & Context",level:"intermediate",nd:"nd801",weight:10},{course_key:"ud876-2",title:"Google Play Services: Analytics",level:"intermediate",nd:"nd801",weight:10},{course_key:"ud876-3",title:"Google Play Services: AdMob",level:"intermediate",nd:"nd801",weight:10},{course_key:"ud876-4",title:"Google Play Services: Maps",level:"intermediate",
nd:"nd801",weight:10},{course_key:"ud876-5",title:"Google Play Services: Identity",level:"intermediate",nd:"nd801",weight:10},{course_key:"ud876-nd",title:"Google Play Services",level:"advanced",nd:"nd801",weight:10},{course_key:"ud882",title:"Responsive Images",level:"intermediate",nd:"nd001",weight:10},{course_key:"ud882-nd",title:"Responsive Images",level:"intermediate",nd:"nd001",weight:10},{course_key:"ud884",title:"Website Performance Optimization",level:"intermediate",nd:"nd001",weight:10},
{course_key:"ud884-nd",title:"Website Performance Optimization",level:"intermediate",nd:"nd001",weight:10},{course_key:"ud893",title:"Responsive Web Design Fundamentals",level:"intermediate",nd:"nd001",weight:10},{course_key:"ud893-nd",title:"Responsive Web Design Fundamentals",level:"intermediate",nd:"nd001",weight:10},{course_key:"ud902",title:"Learn Swift Programming Syntax",level:"intermediate",nd:"nd003",weight:10},{course_key:"ud902-nd",title:"Learn Swift Programming Syntax",level:"intermediate",
nd:"nd003",weight:10},{course_key:"ud904",title:"CS 6300: Software Development Process - Part 3 of 4",level:"intermediate",nd:0,weight:0},{course_key:"ud915",title:"Educational Technology",level:"beginner",nd:0,weight:0},{course_key:"ud919",title:"Model Building and Validation",level:"advanced",nd:"nd002",weight:7},{course_key:"ud923",title:"Introduction to Operating Systems",level:"intermediate",nd:0,weight:0},{course_key:"ud953",title:"Linear Algebra Refresher Course",level:0,nd:0,weight:0},{course_key:"ud955",
title:"Computational Photography",level:"intermediate",nd:0,weight:0},{course_key:"ud957",title:"Software Development Process",level:"intermediate",nd:0,weight:0},{course_key:"ud989",title:"JavaScript Design Patterns",level:"advanced",nd:"nd001",weight:10},{course_key:"ud990",title:"Learn Backbone.js",level:"advanced",nd:"nd001",weight:7}]);angular.module("viewer").service("executor",["mixin","api","dateTime","lessonContentContext","lesson","iframeEvaluator","$location",function(a,c,b,d,e,f,g){var h={submitFiles:function(a,j,m){var l=m.operation||"TEST",k=d.getCurrent(),p=b.getCurrentAsUTC(),q={model:"Submission",operation:l,parts:_.map(a,function(a){return{model:"SubmissionPart",marker:a.name,content:a.content}})},n={submission:q,client_route:"!"+g.path()},r=function(a){h.emitEvent("evaluation",{evaluation:a.data.evaluation,submission:q,
context:k,timestamp:p});return a};h.emitEvent("submit");e.updateLastSubmissionForMorsel(j,q);return m.useIframe?f.evaluate(q,j,m.iframeUrl).then(function(a){n.submission.client_execution=a.execution;n.submission.client_execution.raw_result=JSON.stringify(a.execution.raw_result);return c.post("/nodes/"+j+"/evaluation?_method=GET",n,{timeout:3E4}).then(r)},function(){n.execution={model:"SubmissionExecution",raw_result:"",status:"te"};return c.post("/nodes/"+j+"/evaluation?_method=GET",n,{timeout:3E4}).then(r)}):
c.post("/nodes/"+j+"/evaluation?_method=GET",n,{timeout:3E4}).then(r)}};return a.eventEmission(h,["update","submit","evaluation"])}]);angular.module("viewer").factory("iframeEvaluator",["mixin","$q","$timeout","isTrustedDomain",function(a,c,b,d){var e=null,f=null,g=null,h={evaluate:function(a,d,m){var l=c.defer();e=l;f=a;g=m;h.emitEvent("newEvaluation",{nodeId:d,iframeUrl:m});b(function(){l.reject("timeout")},6E4);return l.promise}};window.addEventListener("message",function(a){if(d(a.origin)){var c;a=a.data;try{c=JSON.parse(a)}catch(m){c=a}"ready"===c.messageType?h.emitEvent("postMessageRequest",{submission:f,iframeUrl:g}):"execution"===
c.messageType&&b(function(){e.resolve(c.result)})}},!1);return a.eventEmission(h,["newEvaluation","postMessageRequest"])}]);angular.module("viewer").factory("imageQuiz",["mixin","api","dateTime","lesson","lessonContentContext","$timeout",function(a,c,b,d,e,f){function g(a,b){a&&_.each(b.widgets,function(b){var c=_.find(a.parts,function(a){return a.marker===b.marker});c&&("RadioButtonWidget"===b.model?c.content&&(l.radioGroupAnswers[b.group]=c.marker):l.answers[b.marker]=c.content)})}var h=null,i=null,j=null,m;d.addEventListener("morselChange",function(a){m=a},{replayLast:!0});var l={answers:null,radioGroupAnswers:null,
setQuiz:function(a){h=a;var b=Math.round((new Date).getTime());i=b;l.answers={};_.each(h.widgets,function(a){l.answers[a.marker]=a.initial_value});l.radioGroupAnswers={};h.hasOwnProperty("lastSubmission")?(g(h.lastSubmission,h),l.emitEvent("ready")):c.get("/nodes/"+a.key+"/state").success(function(a){if(m.key===h.key&&b===i){var c=a.nodestates[0]?a.nodestates[0].last_submission_data:null,a=a.nodestates[0]?a.nodestates[0].bin_codes:null;g(c,h);d.updateLastSubmissionForMorsel(h.key,c);d.mergeBinCodesForMorsel(h.key,
a);l.emitEvent("ready")}}).error(function(a){console.error("Error fetching quiz progress data");console.error(a)})},getQuiz:function(){return h},submit:function(){_.each(h.widgets,function(a){"RadioButtonWidget"===a.model&&(l.answers[a.marker]=l.radioGroupAnswers[a.group]===a.marker)});l.emitEvent("submitting",l.answers);if(h.evaluator){var a=Math.round((new Date).getTime()),g=h.key;j=a;var i=e.getCurrent(),n=b.getCurrentAsUTC(),r={model:"Submission",operation:"GRADE",parts:_.map(l.answers,function(a,
b){return{model:"SubmissionPart",marker:b,content:a}})},u={submission:r};d.updateLastSubmissionForMorsel(h.key,r);c.post("/nodes/"+h.key+"/evaluation?_method=GET",u).success(function(b){l.emitEvent("evaluation",{evaluation:b.evaluation,submission:r,context:i,timestamp:n});if(g===m.key&&a===j)if(b.evaluation){d.mergeBinCodesForMorsel(h.key,b.evaluation.bin_codes);if(b.evaluation.passed||0<b.evaluation.score.points_earned)d.markMorselAsCompleted(g),h.containingExercise&&d.markMorselAsCompleted(h.containingExercise.key);
...
{morselId:h.key,comment:"We encountered an error. Please try submitting again."})})}else f(function(){_.reduce(h.widgets,function(a,b){return a&&l.answers[b.marker]===b.correct_value},!0)?l.emitEvent("correct",{morselId:h.key}):l.emitEvent("incorrect",{morselId:h.key})},10)}};return a.eventEmission(l,"ready submitting incorrect correct recorded error evaluation".split(" "))}]);angular.module("viewer").factory("isTrustedDomain",[function(){return function(a){return"https://www.udacity.com"===a||"http://localhost:8080"===a||"http://localhost:8087"===a||"https://assignments-dot-udacity-extras.appspot.com"===a}}]);angular.module("viewer").factory("lesson",["api","$q","mixin","lessonContentContext","typeForTag","freshContent",function(a,c,b,d,e,f){function g(a,b){var c=_.find(a,function(a){for(var c=b.length-1;0<=c;c--){var d=a.content_context[c];if(!d||d.node_ref.key!==b[c].node_key||d.tag!==b[c].tag)return!1}return!0});c&&(c=_.object(_.map(c.content_context,function(a){return[e[a.tag]+"Key",a.node_ref.key]})));return c}var h=null,i=null,j=null,m=null,l=null,k=null,p=null,q={announcePendingCourseChange:function(){q.emitEvent("pendingCourseChange",
{course:h})},changeCourse:function(b){var e=Math.round((new Date).getTime());p=e;c.all([a.get("/nodes/"+b+"?depth=1&fresh="+(!0===f)+"&required_behavior=view&projection=navigation"),a.get("/nodes/"+b+"/state?fresh=true")]).then(function(a){var c=a[0].data,f=a[1].data,l=a=null;if(e===p){h=c.references.Node[b];i=c.references.Node;h.steps_refs=_.reject(h.steps_refs,function(a){return!i[a.key]});if(c=_.find(f.nodestates,function(a){return a.node_ref.key===b}))(l=g(c.last_interactions,d.getCurrent()))&&
l.lessonKey&&(a=i[l.lessonKey]);q.emitEvent("courseChange",{course:h,lessons:_.map(h.steps_refs,function(a,b){var c=i[a.key];c.indexInCourse=b;return c}),lastLessonVisited:a})}},function(a){e===p&&a.status&&(403===a.status||404===a.status)&&q.emitEvent("coursePermissionDenied",b)})},announcePendingLessonChange:function(){q.emitEvent("pendingLessonChange",{lesson:j})},changeLesson:function(b){var e=Math.round((new Date).getTime());k=e;c.all([a.get("/nodes/"+b+"?depth=2&fresh="+(!0===f)+"&required_behavior=view&projection=classroom"),
a.get("/nodes/"+b+"/state?fresh=true")]).then(function(a){if(e===k){var c=a[0].data,f=a[1].data,a=null;j=c.references.Node[b];m=c.references.Node;j.steps_refs=_.reject(j.steps_refs,function(a){return!m[a.key]});_.each(j.steps_refs,function(a,b){var c=m[a.key];if(!c)throw Error("Expected a morsel but got "+JSON.stringify(c,2));"Exercise"===c.model&&(_.each(["lecture_ref","quiz_ref","answer_ref"],function(a){c[a]&&(c[a]=m[c[a].key],c[a]&&(c[a].containingExercise=c))}),_.each(c.tidbits,function(a){a.tidbit_ref=
m[a.tidbit_ref.key];a.tidbit_ref&&(a.tidbit_ref.containingExercise=c)}));m[a.key].indexInLesson=b});_.each(m,function(a){if("ImageFormQuiz"===a.model||"ProgrammingQuiz"===a.model)a.binCodes=[]});if(c=_.find(f.nodestates,function(a){return a.node_ref.key===b}))_.each(c.completed_descendant_node_refs,function(a){q.markMorselAsCompleted(a.key)}),(c=g(c.last_interactions,d.getCurrent()))&&c.morselKey&&(a=m[c.morselKey]);q.emitEvent("lessonChange",{lesson:j,morsels:_.map(j.steps_refs,function(a){return m[a.key]}),
lastMorselVisited:a})}},function(a){e===k&&a&&(a.status&&(403===a.status||404===a.status))&&q.emitEvent("lessonPermissionDenied",b)})},announcePendingMorselChange:function(a){q.emitEvent("pendingMorselChange",{from:l,to:a})},changeMorsel:function(a){l=m[a];q.emitEvent("morselChange",l)},changeTidbit:function(a){q.emitEvent("tidbitChange",{tidbit:a?m[a]:null})},markMorselAsCompleted:function(a){m[a]?m[a].completed=!0:console.error("Can't find morsel #"+a+" to mark it as completed")},updateProgressForMorsel:function(a,
b){m[a]&&m[b]?m[a].progressNodeId=b:console.error("Can't find container morsel #"+a+" or perhaps progress morsel #"+b+" to update progress")},updateLastSubmissionForMorsel:function(a,b){m[a]?m[a].lastSubmission=b:console.error("Can't find morsel #"+a+"to update activity")},mergeBinCodesForMorsel:function(a,b){m[a]&&(m[a].binCodes=_.union(m[a].binCodes,b))}};return b.eventEmission(q,"pendingCourseChange courseChange pendingLessonChange lessonChange pendingMorselChange morselChange tidbitChange lessonPermissionDenied coursePermissionDenied".split(" "))}]);angular.module("viewer").factory("lessonPrefetcher",["$http","$timeout","lesson",function(a,c,b){var d=null;b.addEventListener("courseChange",function(a){d=a.lessons},{replayLast:!0});b.addEventListener("courseChange",function(){b.addEventListener("lessonChange",function(b){var f=_.find(d,function(a){return a.key===b.lesson.key}).indexInCourse;if(0<f){var g=d[f-1].key;c(function(){a.get("/api/nodes/"+g+"?depth=2&fresh=false&required_behavior=view&projection=classroom")},1E4,!1)}if(0<=f&&f<d.length-
...
d.key+"/state").success(function(a){d.lastSubmission||b.updateLastSubmissionForMorsel(d.key,a.nodestates[0]?a.nodestates[0].last_submission_data:null)}).error(function(){console.error("Failed to prefetch last submission for quiz with id "+d.key)})}});return{}}]);angular.module("viewer").factory("lessonActivityTracker",["$http","lesson","lessonContentContext","youtubeVideoPlayer","dateTime","imageQuiz","executor","mixin","$routeParams",function(a,c,b,d,e,f,g,h){function i(a){function b(a){return l(_.map(_.without([a.submission,a.evaluation],null,void 0),function(b){return{context:a.context,data:b}}),{chain:!0,timestamp:a.timestamp})}function c(){d+=1;3>d?b(a).then(function(){},c):k.emitEvent("quizActivityLogFailure",{context:a.context})}var d=0;a.evaluation&&
(a.evaluation.model="SubmissionEvaluation");b(a).then(function(){},c)}var j=null,m=null,l=function(b,c){var c=c||{},d={items:_.map(b,function(a){return{occurrence_time:c.timestamp||e.getCurrentAsUTC(),content_context:_.without(a.context,null,void 0,"none"),data:a.data}}),current_time:e.getCurrentAsUTC()},f="/api/activity";c.chain&&(f+="?related_chain=true");return a.post(f,d).then(function(a){200!==a.status&&(console.error("Error when trying to post activity. Response:"),console.error(a))})};d.addEventListener("videoPlay",
function(a){j=a},{replayLast:!0});d.addEventListener("videoPause",function(a){null!==j&&(null!==a&&void 0!==a)&&(m=a,j>m?j=a:l([{context:b.getCurrent(),data:{model:"PlayableView",start:j,stop:m}}]));j=m=null},{replayLast:!0});f.addEventListener("evaluation",i,{replayLast:!0});g.addEventListener("evaluation",i,{replayLast:!0});c.addEventListener("morselChange",function(a){if(!a.completed&&("Reading"===a.model||"EmbeddedFrame"===a.model))c.markMorselAsCompleted(a.key),l([{context:b.getCurrent(),data:{model:"CompletedExperience",
completion_context:"Student viewed the node"}}])});var k={logNodeVisit:function(a){l([{context:b.getCurrent(a),data:{model:"NodeVisit"}}])}};return h.eventEmission(k,["quizActivityLogFailure"])}]);angular.module("viewer").factory("lessonContentContext",["$routeParams","tags",function(a,c){return{getCurrent:function(b){var d=["course","lesson","exercise","morsel","tidbit"];b&&(b=_.indexOf(d,b),0<=b&&(d=d.slice(0,b+1)));return _.without(_.map(d,function(b){if(a[b+"Key"]&&"none"!==a[b+"Key"])return{tag:c[b],node_key:a[b+"Key"]}}),void 0,null)}}}]);angular.module("viewer").factory("programmingQuiz",["api","mixin","codeEditor","executor","lesson",function(a,c,b,d,e){function f(a){return _.map(a,function(a){return{name:a.name,content:a.text}})}function g(a){return _.map(a,function(a){var b={name:a.name};a._content&&"text/plain"===a._content._content_type?(b.text=a._content._plaintext,b.kind="plaintext"):a._content&&-1!==a._content._content_type.indexOf("image")&&(b.servingUrl=a._content._serving_url,b.kind="image");return b})}function h(a,b){a&&
_.each(b,function(b){var c=_.find(a.parts,function(a){return a.marker===b.name});c&&(b.text=c.content)})}var i=null,j=null,m=null,l;e.addEventListener("morselChange",function(a){l=a},{replayLast:!0});var k=null,p=null,q=null,n=null,r={init:function(c){var d=Math.round((new Date).getTime()),f=c.key;i=d;p=g(c.initial_code_files);k=g(c.initial_code_files);q=c.key;n=c;c.hasOwnProperty("lastSubmission")?(h(c.lastSubmission,k),b.loadFiles(k).then(function(){r.emitEvent("ready")})):a.get("/nodes/"+c.key+
"/state").success(function(a){if(f===l.key&&d===i){var c=a.nodestates[0],a=c?c.last_submission_data:null,c=c?c.bin_codes:[];h(a,k);e.updateLastSubmissionForMorsel(q,a);e.mergeBinCodesForMorsel(q,c);b.loadFiles(k);r.emitEvent("ready")}}).error(function(a){console.error("Error fetching programming quiz progress data");console.error(a)})},submit:function(){var a=Math.round((new Date).getTime()),c=l.key;j=a;var g={operation:"GRADE",useIframe:l.evaluator&&"IFrameEvaluator"===l.evaluator.model,iframeUrl:l.evaluator&&
l.evaluator.external_uri};r.emitEvent("submitting",{morselId:q,submissionOptions:g});d.submitFiles(f(b.getAllFiles()),q,g).then(function(b){b=b.data;if(a===j&&c===l.key){e.mergeBinCodesForMorsel(c,b.evaluation.bin_codes);if(b.evaluation.passed||0<b.evaluation.score.points_earned)e.markMorselAsCompleted(c),n.containingExercise&&e.markMorselAsCompleted(n.containingExercise.key);b.evaluation.comment&&b.evaluation.comment.indexOf("est case input")&&(r.emitEvent("run",{morselId:c,result:{stdout:b.evaluation.comment,
stderr:null,images:null}}),b.evaluation.comment=b.evaluation.feedback?b.evaluation.feedback:"Please see detailed feedback in the output area below.");r.emitEvent("evaluated",{quiz:l,result:b.evaluation})}},function(a){r.emitEvent("evaluationError",a.data)})},run:function(){var a=Math.round((new Date).getTime()),c=l.key;m=a;d.submitFiles(f(b.getAllFiles()),q,{operation:"TEST",useIframe:l.evaluator&&"IFrameEvaluator"===l.evaluator.model,iframeUrl:l.evaluator&&l.evaluator.external_uri}).then(function(b){b=
b.data;if(a===m&&c===l.key){var d={};b.execution&&b.execution._output_attachments&&(d=b.execution._output_attachments,b=g(b.execution._ordered_file_attachments),d={stdout:d.stdout?d.stdout._content._plaintext:null,stderr:d.stderr?d.stderr._content._plaintext:null,images:_.filter(b,function(a){return"image"===a.kind})});r.emitEvent("run",{morselId:c,result:d})}},function(a){r.emitEvent("runError",a.data)})},reset:function(){b.loadFiles(p)}};return c.eventEmission(r,"submitting evaluated evaluationError run runError ready".split(" "))}]);angular.module("viewer").factory("tidbits",[function(){return{isTidbitRelevant:function(a,c,b){return _.some(a.bin_code_groups,function(a){return(!b||a.bin_codes.length)&&_.every(a.bin_codes,function(a){return _.contains(c,a)})})},getTidbitViewerMode:function(a){return a?"tidbit-only":"quiz-only"}}}]);angular.module("viewer").constant("viewerPartialMap",{Video:{prefix:"video"},SketchVideo:{prefix:"video"},ProgrammingQuiz:{prefix:"programming_quiz",extension:!0},ImageFormQuiz:{prefix:"image_quiz"},Sketch:{prefix:"sketch"},EmbeddedFrame:{prefix:"iframe"},Reading:{prefix:"reading"}});angular.module("viewer").factory("viewerSettings",["mixin",function(a){var c={autoPlay:!0},b={set:function(a,e){if(c.hasOwnProperty(a)){var f=c[a];c[a]=e;b.emitEvent("change",{name:a,oldValue:f,newValue:e})}else console.error("can't set invalid setting: "+a)},get:function(a){if(c.hasOwnProperty(a))return c[a];console.error("Can't get invalid setting: "+a)}};return a.eventEmission(b,["change"])}]);angular.module("widgets").factory("github",["$q","libraryLoader","$rootScope","cookieJar",function(a,c,b,d){var e=c.loadLibraries(["base64"]),f={Accept:"application/vnd.github.raw","Content-Type":"application/json"},g={scopes:["repo"],note:"Udacity Issue Tracker",client_id:"bb427a8f7beb27b582e0",client_secret:"a014d61f827e1a761898d216ce44e23431237973"},h={scopes:["repo"],note:"Udacity Github Tracker",client_secret:"a014d61f827e1a761898d216ce44e23431237973"},i=null;(c=d.get("githubIssueTrackerToken"))&&
(i=c);var j={setToken:function(a){null===a&&d.remove("githubIssueTrackerToken");i=a},hasToken:function(){return null!==i},getAuth:function(c,g,i){var p=a.defer();e.then(function(){var a=angular.copy(f);a.Authorization="Basic "+Base64.encode(c+":"+g);i&&(a["X-GitHub-OTP"]=i);$.ajax("https://api.github.com/authorizations/clients/bb427a8f7beb27b582e0",{type:"PUT",data:JSON.stringify(h),headers:a}).done(function(a){null===a.token?j._postAuth(c,g,i).then(function(a){p.resolve(a)},function(a){p.reject(a)}):
(j.setToken(a.token),d.set("githubIssueTrackerToken",a.token),p.resolve(a))}).fail(function(a){p.reject(a)}).always(function(){b.$apply()})});return p.promise},_postAuth:function(c,h,i){var p=a.defer();e.then(function(){var a=angular.copy(f);a.Authorization="Basic "+Base64.encode(c+":"+h);i&&(a["X-GitHub-OTP"]=i);$.ajax("https://api.github.com/authorizations",{type:"POST",data:JSON.stringify(g),headers:a}).done(function(a){j.setToken(a.token);d.set("githubIssueTrackerToken",a.token);p.resolve(a)}).fail(function(a){p.reject(a)}).always(function(){b.$apply()})});
...
{projection:"image"}).then(function(b){b&&b._image_url&&a.partners[d].push({imageUrl:b._image_url,name:b.name})})})};a.fetchCards=function(b,d){return c.batchGet(b,{fresh:!1}).then(function(c){return _.without(_.map(b,function(b){var e=!1,j=c[b];if(c[b]){var m=null,l=j.catalog_entry;l&&l._image&&(m=l._image.serving_url);d&&(j.affiliate_program_refs&&0<j.affiliate_program_refs.length)&&"udacity"!==j.affiliate_program_refs[0].key&&(e=!0,a._fetchAffiliate(j.affiliate_program_refs,b));return{key:b,_slug:j._slug,
servingUrl:m,title:j.title,isNanodegree:j.isNanodegree(),isCareer:j.isCareer(),getCourseType:j.getCourseType(),showPartner:e&&d}}return null}),null)})};a._setLengthOfRow=function(a,b){return!b||6<b?6>=a?a:0===a%6?6:0===a%5?5:0===a%4?4:0===a%3?3:6:b};a.setUpCards=function(b,c,g,h,i){b?(a.lengthOfRow=a._setLengthOfRow(b.length,c),a.columnWidthClass="col-xs-"+a._calculateColumnWidth(a.lengthOfRow),a.cardImageSize=[1200/a.lengthOfRow,0],angular.isArray(h)&&2===h.length?a.cardImageSize=h:angular.isNumber(h)&&
...
_.find(a.get("_enrollments"),function(a){return a.node_key===g.key}),i=i&&b.isCurrentEnrolled(i,g),k;k="pad"===j.bannerStyle?c.exists("upsell-cancel-banner-progress"):c.exists("upsell-cancel-banner-viewer");d.showBanner=!k&&d.course.catalog_entry.paid_class&&d.course.catalog_entry.paid_class.open_for_enrollment&&!i;d.learnMore="/success#!/?courseKey="+f.encodeURIComponent(g.key)+"&start="+f.encodeURIComponent(e.absUrl())}else d.showBanner=!1});d.goToPurchase=function(a){f.location.href="/checkout#!/0?products="+
